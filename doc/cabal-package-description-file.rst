...
.. note::
    ...
    Listing modules in :pkg-field:`autogen-modules` exempts ``cabal sdist`` from
    finding their source files. Creating a source distribution with ``cabal
    sdist`` reports any source files not found and fails.
