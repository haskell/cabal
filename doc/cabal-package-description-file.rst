...
.. [#old-style-build-tool-depends]
   ...
   this has a few major drawbacks.  Using Nix-style builds it's considered an
   error if you depend on a exe-only package via build-depends: the solver will
   refuse it.  It may or may not place the executable on ``PATH``.  It does not
   ensure the correct version of the package is installed, so you might end up
   overwriting versions with each other.
