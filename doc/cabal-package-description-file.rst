...
.. pkg-field:: build-depends: library list
    ...
    **Library Names**
    ...
    .. Note::
       ...
       From version 1.8 of the Cabal specification, the
       :pkg-field:`build-depends` field of an executable, test suite (from
       version 1.10) or benchmark (from version 1.14) section can specify the
       (or, if applicable, a) library in the same package by the name of the
       library (without a version constraint). Cabal then treats the executable,
       test suite or benchmark as if it were in another package that depended on
       the package.
