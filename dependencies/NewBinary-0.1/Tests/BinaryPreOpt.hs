...
newtype Bin a = BinPtr (Int, Int) -- byte/bit
...
--  state <- newWriteState mod
  return (BinIO undefined r h o c)
...
openBinMem size mod
 ...
 | otherwise = do
   ...
   c <- newZeroInt
--   state <- newWriteState mod
   return (BinMem undefined ix_r sz_r arr_r o c)
...
tellBin (BinIO _ r _ o _)   =  do ix <- readFastMutInt r; bix <- readFastMutInt o; return (BinPtr (ix,bix))
tellBin (BinMem _ r _ _ o _) = do ix <- readFastMutInt r; bix <- readFastMutInt o; return (BinPtr (ix,bix))



seekBin :: BinHandle -> Bin a -> IO ()
seekBin bh@(BinIO _ ix_r h o c) (BinPtr (p,bit)) = do 
  ...
  getBits bh bit
  return ()
seekBin h@(BinMem _ ix_r sz_r a o c) (BinPtr (p,bit)) = do
...
isEOFBin (BinMem _ ix_r sz_r a _ _) = do
  ix <- readFastMutInt ix_r
  sz <- readFastMutInt sz_r
  return (ix >= sz)
isEOFBin (BinIO _ ix_r h _ _) = hIsEOF h

writeBinMem :: BinHandle -> FilePath -> IO ()
writeBinMem (BinIO _ _ _ _ _) _ = error "Data.Binary.writeBinMem: not a memory handle"
writeBinMem bh@(BinMem _ ix_r sz_r arr_r bit_off_r bit_cache_r) fn = do
  flushByte bh
...
readBinMem filename = do
  ...
  return (BinMem undefined {-initReadState-} ix_r sz_r arr_r bit_off_r bit_cache_r)

-- expand the size of the array to include a specified offset
expandBin :: BinHandle -> Int -> IO ()
expandBin (BinMem _ ix_r sz_r arr_r _ _) off = do
   sz <- readFastMutInt sz_r
...
--   hPutStrLn stderr ("expanding to size: " ++ show sz')
   return ()
expandBin (BinIO _ _ _ _ _) _ = return ()
...
putWord8 h@(BinMem _ ix_r sz_r arr_r bit_off_r bit_cache_r) w = do
	...
	then do expandBin h ix
		...
		unsafeWrite arr ix w
    		...
    		return ()
putWord8 bh@(BinIO _ ix_r h bit_off_r bit_cache_r) w = do
    bit_off <- readFastMutInt bit_off_r
...
getWord8 h@(BinMem _ ix_r sz_r arr_r bit_off_r _) = do
    bit_off <- readFastMutInt bit_off_r
    ...
    writeFastMutInt ix_r (ix+1)
    return w
getWord8 bh@(BinIO _ ix_r h bit_off_r _) = do
    bit_off <- readFastMutInt bit_off_r
