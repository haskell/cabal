...
main = do
    cabalTest $ do
        ...
        assertOutputContains "X.c Y.c" res
        cabal' "clean" []
        ...
        assertOutputDoesNotMatch "GHC response file arguments.* -c .*-jsem.*X\\.c Y\\.c" res
        assertOutputMatches "GHC response file arguments.* -c .*X\\.c Y\\.c" res
        cabal' "clean" []
        -- cabal's `-j4` isn't supposed to leak into GHC's `-j4`
        res <- cabalG' ["-j4"] "build" ["-v2"]
        assertOutputDoesNotMatch "GHC response file arguments.* -c .*-j4.*X\\.c Y\\.c" res
        assertOutputMatches "GHC response file arguments.* -c .*X\\.c Y\\.c" res
