...
import Control.Monad (forM_, liftM2, liftM4, unless, void, when)
...
import System.Directory (canonicalizePath, copyFile, createDirectoryIfMissing, doesDirectoryExist, doesFileExist, getCurrentDirectory, getDirectoryContents, getTemporaryDirectory, listDirectory)
import System.Exit (ExitCode (..))
import System.FilePath (dropTrailingPathSeparator, joinPath, normalise, splitFileName, splitPath, takeDirectory, takeDrive, takeExtensions, (<.>), (</>))
...
shouldDirectoryNotExist path =
  withFrozenCallStack $
    liftIO $
      doesDirectoryExist path >>= assertBool (path ++ " should exist") . not
