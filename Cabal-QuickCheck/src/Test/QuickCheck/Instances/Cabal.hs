...
import Data.Char (isAlphaNum, isDigit, isLetter, toLower)
...
instance Arbitrary PackageName where
  arbitrary = mkPackageName . intercalate "-" <$> shortListOf1 2 nameComponent
    where
      nameComponent =
        shortListOf1 5 (elements packageChars)
...
instance Arbitrary Version where
  ...
  shrink ver =
    [ mkVersion ns | ns <- shrink (versionNumbers ver), not (null ns)
    ]
...
instance Arbitrary ModuleName where
  arbitrary = fromString . intercalate "." <$> shortListOf1 4 comp
    where
...
instance Arbitrary FlagName where
  arbitrary =
    ...
    where
      flagident =
        lowercase
          <$> shortListOf1 5 (elements flagChars)
            `suchThat` (("-" /=) . take 1)
      flagChars = "-_" ++ ['a' .. 'z']
...
instance Arbitrary PkgconfigVersion where
  arbitrary = PkgconfigVersion . BS8.pack . dropDash . concat <$> listOf1 elems
    where
      elems =
        frequency
          [ (2, pure ".")
...
arbitraryShortPath = arbitraryShortStringWithout "{}[],<>:|*?" `suchThat` (not . winDevice)
  where
    ...
    splitBy _ "" = []
    splitBy seps str =
      let (part, rest) = break (`elem` seps) str
