    ...
    getBody pkg = peekField >>= \mf -> case mf of
      Just (Section line_no sec_type sec_label sec_fields)
        | sec_type == "executable" -> do
            ...
            return (repos, flags, csetup, mlib, sub_libs, (mkUnqualComponentName exename, flds): exes, tests, bms)
        ...
        | sec_type == "library" -> do
            ...
            case mb_libname of
                Just libname ->
                    return (repos, flags, csetup, mlib, (mkUnqualComponentName libname, flds) : sub_libs, exes, tests, bms)
                Nothing -> do
