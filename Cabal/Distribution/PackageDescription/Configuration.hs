...
resolveWithFlags dom enabled os arch impl constrs trees checkDeps =
    either (Left . fromDepMapUnion) Right $ explore (build Map.empty dom)
  where
    ...
    build assigned ((fn, vals) : unassigned) =
        Node assigned $ map (\v -> build (Map.insert fn v assigned) unassigned) vals
    ...
    env flags flag = (maybe (Left flag) Right . Map.lookup flag) flags
...
finalizePD userflags enabled satisfyDep
  ...
  where
    ...
    d2c manual n b = case Map.lookup n userflags of
                     Just val -> [val]
                     Nothing
                      | manual -> [b]
                      | otherwise -> [b, not b]
