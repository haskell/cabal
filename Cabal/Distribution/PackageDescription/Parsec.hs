...
import Distribution.ParseUtils hiding (Field(..), parseFields, readFields)
import Distribution.Parsec.Parser
import Distribution.Parsec.LexerMonad (Position(..), showPos)
...
stanzas :: [Field a] -> [[Field a]]
stanzas [] = []
stanzas (f:fields) = (f:this) : stanzas rest
  where
    (this, rest) = break isStanzaHeader fields

isStanzaHeader :: Field a -> Bool
isStanzaHeader (Field _ (Name _ f) _) = f == "executable"
isStanzaHeader _ = False

------------------------------------------------------------------------------


mapSimpleFields :: (Field a -> ParseResult (Field b)) -> [Field a]
                -> ParseResult [Field b]
mapSimpleFields f = mapM walk
  where
    walk fld@Field{} = f fld
    walk (IfElseBlock sa fs1 fs2) = do
      fs1' <- mapM walk fs1
      fs2' <- mapM walk fs2
      return (IfElseBlock sa fs1' fs2')
    walk (Section n sa fs1) = do
      fs1' <-  mapM walk fs1
      return (Section n sa fs1')
...
-- field descriptor.
parseConstraint :: Field Position -> ParseResult [DependencyWithRenaming]
parseConstraint (Field (Name pos n) ls)
    | n == "build-depends" = runP (posLine pos) n (parseCommaList parse) (fieldLinesToString ls)
...
parsePackageDescription file = do
    ...
    let cabalVersionNeeded =
          head $ [ minVersionBound versionRange
                 | Just versionRange <- [ simpleParse (fieldLinesToString ls)
                                        | Field _ (Name _ "cabal-version") ls <- fields0 ] ]
              ++ [Version [0] []]
