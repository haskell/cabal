...
-- | A 'ComponentInstanceId' uniquely identifies the transitive source
...
instance Binary ComponentInstanceId

instance Text ComponentInstanceId where
  disp (ComponentInstanceId str) = text str

  parse = ComponentInstanceId `fmap` Parse.munch1 abi_char
   where abi_char c = Char.isAlphaNum c || c `elem` "-_."

instance NFData ComponentInstanceId where
    rnf (ComponentInstanceId pk) = rnf pk

-- | Returns library name prefixed with HS, suitable for filenames
getHSLibraryName :: ComponentInstanceId -> String
getHSLibraryName (ComponentInstanceId s) = "HS" ++ s
...
class Package pkg => HasComponentInstanceId pkg where
  installedSourceHash :: pkg -> ComponentInstanceId
...
class (HasComponentInstanceId pkg) => PackageInstalled pkg where
  installedDepends :: pkg -> [ComponentInstanceId]
