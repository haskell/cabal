...
-- Older installed package info files did not have the installedSourceHash
-- field, so if it is missing then we fill it as the source package ID.
setComponentInstanceId :: InstalledPackageInfo -> InstalledPackageInfo
setComponentInstanceId pkginfo@InstalledPackageInfo {
                        installedSourceHash = ComponentInstanceId "",
                        sourcePackageId    = pkgid
                      }
                    = pkginfo {
                        --TODO use a proper named function for the conversion
                        -- from source package id to installed package id
                        installedSourceHash = ComponentInstanceId (display pkgid)
                      }
setComponentInstanceId pkginfo = pkginfo
...
-- Note in particular that it does not include the 'ComponentInstanceId', just
