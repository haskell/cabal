...
import qualified Distribution.InstalledPackageInfo as InstalledPackageInfo
                                ( InstalledPackageInfo(..) )
import Distribution.Package ( ComponentInstanceId(..), getHSLibraryName )
...
buildOrReplLib forRepl verbosity numJobs _pkg_descr lbi lib clbi = do
  let libName@(ComponentInstanceId cname) = componentSourceHash clbi
...
installLib verbosity lbi targetDir dynlibTargetDir builtDir _pkg lib clbi = do
  ...
  where
    ...
    cid = compilerId (compiler lbi)
    libName = componentSourceHash clbi
