...
import Distribution.Simple.Compiler
    ...
    , installedSourceHashSupported, renamingPackageFlagsSupported
    , unifiedIPIDRequired )
...
import Distribution.Package
    ...
    , ComponentInstanceId(..), thisPackageVersion, ComponentInstanceId(..) )
  ...
  = do  let distPref = fromFlag (configDistPref cfg)
        ...
        let pid = packageId pkg_descr0
            internalPackage = emptyInstalledPackageInfo {
                ...
                --      ComponentInstanceId . display here is a hack.
                Installed.installedSourceHash =
                   ComponentInstanceId $ display $ pid,
        ...
        let installDeps = Map.elems
                        . Map.fromList
                        . map (\v -> (Installed.installedSourceHash v, v))
                        $ externalPkgDeps ++ holeDeps

        packageDependsIndex <-
          case PackageIndex.dependencyClosure installedPackageSet
                  (map Installed.installedSourceHash installDeps) of
        ...
        let pseudoTopPkg = emptyInstalledPackageInfo {
                Installed.installedSourceHash =
                   ComponentInstanceId (display (packageId pkg_descr)),
                Installed.sourcePackageId = packageId pkg_descr,
                Installed.depends =
                  map Installed.installedSourceHash installDeps
              }
...
combinedConstraints :: [Dependency] ->
                       [(PackageName, ComponentInstanceId)] ->
                       InstalledPackageIndex ->
                       Either String ([Dependency],
                                      Map PackageName InstalledPackageInfo)
combinedConstraints constraints dependencies installedPackages = do

    when (not (null badComponentInstanceIds)) $
      Left $ render $ text "The following package dependencies were requested"
         $+$ nest 4 (dispDependencies badComponentInstanceIds)
  ...
  where
    ...
    dependenciesPkgInfo :: [(PackageName, ComponentInstanceId,
                             Maybe InstalledPackageInfo)]
    dependenciesPkgInfo =
      [ (pkgname, ipkgid, mpkg)
      | (pkgname, ipkgid) <- dependencies
      , let mpkg = PackageIndex.lookupComponentInstanceId
...
configureInstantiateWith pkg_descr cfg installedPackageSet = do
        ...
        let selectHoleDependency (k,(i,m)) =
              case PackageIndex.lookupComponentInstanceId installedPackageSet i of
...
-- | This method computes a default, "good enough" 'ComponentInstanceId'
...
computeComponentInstanceId :: PackageDescription
            -> ComponentName
            -- TODO: careful here!
            -> [ComponentInstanceId] -- IPIDs of the component dependencies
            -> FlagAssignment
            -> IO ComponentInstanceId
computeComponentInstanceId pkg_descr cname dep_ipids flagAssignment = do
...
mkComponentsLocalBuildInfo cfg comp installedPackages pkg_descr
    ...
    lib_hash@(ComponentInstanceId str) <-
      ...
      case configIPID cfg of
        Flag lib_hash0 ->
            ...
            let env = packageTemplateEnv (package pkg_descr) (ComponentInstanceId "")
                str = fromPathTemplate (substPathTemplate env (toPathTemplate lib_hash0))
            in return (ComponentInstanceId str)
        _ ->
          computeComponentInstanceId pkg_descr CLibName (getDeps CLibName) flagAssignment
    let extractCandidateCompatKey s
        ...
        cand_compat_key = ComponentInstanceId (extractCandidateCompatKey str)
        old_style_key = ComponentInstanceId (display (package pkg_descr))
        best_key = ComponentInstanceId str
        compat_key =
            if installedSourceHashSupported comp
  ...
  where
    getDeps cname =
          ...
          in map Installed.installedSourceHash externalPkgs
    ...
    componentLocalBuildInfo lib_hash compat_key component =
        ...
        let exports = map (\n -> Installed.ExposedModule n Nothing Nothing)
            ...
            esigs = map (\n -> Installed.ExposedModule n Nothing
                                (fmap (\(pkg,m) -> Installed.OriginalModule
                                                      (Installed.installedSourceHash pkg) m)
      ...
      where
        ...
        cpds = if newPackageDepsBehaviour pkg_descr
               then dedup $
                    [ (Installed.installedSourceHash pkg, packageId pkg)
                    | pkg <- selectSubset bi externalPkgDeps ]
                 ++ [ (lib_hash, pkgid)
                    | pkgid <- selectSubset bi internalPkgDeps ]
               else [ (Installed.installedSourceHash pkg, packageId pkg)
                    | pkg <- externalPkgDeps ]
  ...
  where
    ...
    visibleModules =
      Map.fromListWith (++) $
        ...
        | let directDeps = Set.fromList (map Installed.installedSourceHash externalPkgDeps)
        , pkg <- PackageIndex.allPackages installedPackages
        , Installed.installedSourceHash pkg `Set.member` directDeps
    ...
    visibleModuleDetails pkg = do
        ...
        -- In this case the reexport will point to this package.
            Nothing -> return exposedModule { Installed.exposedReexport =
                            Just (Installed.OriginalModule (Installed.installedSourceHash pkg)
