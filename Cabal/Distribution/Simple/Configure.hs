...
configure (pkg_descr0', pbi) cfg = do
    ...
    info verbosity $ "Finalized package description:\n"
                  ++ showPackageDescription pkg_descr
    -- NB: showPackageDescription does not display the AWFUL HACK GLOBAL
    -- buildDepends, so we have to display it separately
    info verbosity $ "Finalized build-depends: "
    ...
    --
    -- TODO: Move this into a helper function.
    defaultDirs0 :: InstallDirTemplates
  ...
  where
    ...
    componentDeps component =
      ...
      ++ [ cname
         ...
         , Just cname <- [Map.lookup pkgname internalPackageSet] ]
      where
