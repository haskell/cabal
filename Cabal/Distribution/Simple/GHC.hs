...
buildOrReplLib forRepl verbosity numJobs pkg_descr lbi lib clbi = do
  let libName = componentSourceHash clbi
...
registerPackage verbosity installedPkgInfo _pkg lbi inplace packageDbs = do
  ...
  when (not inplace) $ do
    ...
    let key = InstalledPackageInfo.installedSourceHash installedPkgInfo
    when (any ((==key) . InstalledPackageInfo.installedSourceHash) old_pkgs)
