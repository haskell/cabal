...
import Distribution.PackageDescription as PD
         ...
         , allExtensions, libModules, exeModules, flibModules, flibIsShared
...
gbuildSources tmpDir bm =
    case bm of
      ...
      GBuildFLib flib -> flibSources flib
      GReplFLib  flib -> flibSources flib
  where
    ...
    flibSources :: ForeignLib -> IO ([FilePath], [FilePath], [ModuleName])
    flibSources flib@ForeignLib{foreignLibBuildInfo = bnfo} =
      return (cSources bnfo, [], flibModules flib)
...
gbuild verbosity numJobs _pkg_descr lbi bm clbi = do
  ...
  -- build executables
      ...
      linkerOpts = mempty {
                      ...
                      ghcOptLinkFrameworks = toNubListR $ PD.frameworks bnfo,
                      ghcOptInputFiles     = toNubListR
                                             [tmpDir </> x | x <- cObjs]
                    }
  ...
  case bm of
    ...
    GBuildExe _ -> do
      let linkOpts = commonOpts
                   ...
                   `mappend` (if withStaticExe then mempty else dynLinkerOpts)
...
extractRtsInfo lbi =
    case PackageIndex.lookupPackageName (installedPkgs lbi) (PackageName "rts") of
      [(_, [rts])] -> aux rts
...
installFLib verbosity lbi targetDir builtDir _pkg flib =
    install (flibIsShared flib)
            (builtDir </> foreignLibName flib)
            targetDir
            (flibTargetName lbi flib)
  where
