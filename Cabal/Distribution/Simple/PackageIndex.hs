...
import Distribution.Package
         ...
         , HasComponentInstanceId(..), PackageInstalled(..)
         , ComponentInstanceId )
...
--
-- Packages are uniquely identified in by their 'ComponentInstanceId', they can
...
data PackageIndex a = PackageIndex
  ...
  -- by its ComponentInstanceId.
  --
  !(Map ComponentInstanceId a)
  ...
  -- of the same package version. These are unique by ComponentInstanceId
...
instance HasComponentInstanceId a => Monoid (PackageIndex a) where
...
invariant :: HasComponentInstanceId a => PackageIndex a -> Bool
invariant (PackageIndex pids pnames) =
     map installedSourceHash (Map.elems pids)
  == sort
     [ assert pinstOk (installedSourceHash pinst)
     | (pname, pvers)  <- Map.toList pnames
     , let pversOk = not (Map.null pvers)
     , (pver,  pinsts) <- assert pversOk $ Map.toList pvers
     , let pinsts'  = sortBy (comparing installedSourceHash) pinsts
           pinstsOk = all (\g -> length g == 1)
                          (groupBy (equating installedSourceHash) pinsts')
...
mkPackageIndex :: HasComponentInstanceId a
               => Map ComponentInstanceId a
               -> Map PackageName (Map Version [a])
               -> PackageIndex a
...
--
-- If there are duplicates by 'ComponentInstanceId' then later ones mask earlier
-- ones.
--
fromList :: HasComponentInstanceId a => [a] -> PackageIndex a
fromList pkgs = mkPackageIndex pids pnames
  where
    pids      = Map.fromList [ (installedSourceHash pkg, pkg) | pkg <- pkgs ]
    pnames    =
      Map.fromList
        ...
        , let pvers =
                ...
                [ (packageVersion (head pkgsNV),
                   nubBy (equating installedSourceHash) (reverse pkgsNV))
...
--
merge :: HasComponentInstanceId a => PackageIndex a -> PackageIndex a
      -> PackageIndex a
merge (PackageIndex pids1 pnames1) (PackageIndex pids2 pnames2) =
  ...
  where
    ...
    (\\) = deleteFirstsBy (equating installedSourceHash)
...
--
insert :: HasComponentInstanceId a => a -> PackageIndex a -> PackageIndex a
insert pkg (PackageIndex pids pnames) =
    mkPackageIndex pids' pnames'

  where
    pids'   = Map.insert (installedSourceHash pkg) pkg pids
    ...
    insertPackageInstance pkgs =
      pkg : deleteBy (equating installedSourceHash) pkg pkgs


-- | Removes a single installed package from the index.
--
deleteComponentInstanceId :: HasComponentInstanceId a
                         => ComponentInstanceId -> PackageIndex a
                         -> PackageIndex a
deleteComponentInstanceId ipkgid original@(PackageIndex pids pnames) =
  ...
  where
    ...
    deletePkgInstance =
      ...
      . List.deleteBy (\_ pkg -> installedSourceHash pkg == ipkgid) undefined


-- | Removes all packages with this source 'PackageId' from the index.
--
deleteSourcePackageId :: HasComponentInstanceId a => PackageId -> PackageIndex a
                      -> PackageIndex a
  ...
  case Map.lookup (packageName pkgid) pnames of
    ...
    Just pvers  -> case Map.lookup (packageVersion pkgid) pvers of
      ...
      Just pkgs -> mkPackageIndex
                     (foldl' (flip (Map.delete . installedSourceHash)) pids pkgs)
...
--
deletePackageName :: HasComponentInstanceId a => PackageName -> PackageIndex a
                  -> PackageIndex a
deletePackageName name original@(PackageIndex pids pnames) =
  case Map.lookup name pnames of
    Nothing     -> original
    Just pvers  -> mkPackageIndex
                     (foldl' (flip (Map.delete . installedSourceHash)) pids
...
--
allPackagesBySourcePackageId :: HasComponentInstanceId a => PackageIndex a
                             -> [(PackageId, [a])]
...
--
-- Since multiple package DBs mask each other by 'ComponentInstanceId',
-- then we get back at most one package.
--
lookupComponentInstanceId :: PackageIndex a -> ComponentInstanceId
                         -> Maybe a
lookupComponentInstanceId (PackageIndex pids _) pid = Map.lookup pid pids


-- | Does a lookup by source package id (name & version).
--
-- There can be multiple installed packages with the same source 'PackageId'
-- but different 'ComponentInstanceId'. They are returned in order of
...
dependencyCycles index =
  ...
  where
    adjacencyList = [ (pkg, installedSourceHash pkg, installedDepends pkg)
...
brokenPackages index =
  ...
  , let missing = [ pkg' | pkg' <- installedDepends pkg
                         , isNothing (lookupComponentInstanceId index pkg') ]
  , not (null missing) ]
...
dependencyClosure index pkgids0 = case closure mempty [] pkgids0 of
 ...
 where
    ...
    closure completed failed (pkgid:pkgids) = case lookupComponentInstanceId index pkgid of
      Nothing   -> closure completed (pkgid:failed) pkgids
      Just pkg  -> case lookupComponentInstanceId completed (installedSourceHash pkg) of
...
dependencyGraph index = (graph, vertex_to_pkg, id_to_vertex)
  where
    ...
    vertices         = zip (map installedSourceHash pkgs) [0..]
...
dependencyInconsistencies index =
  ...
  where -- for each PackageName,
        ...
        inverseIndex = Map.fromListWith (Map.unionWith (\(a,b) (_,b') -> (a,b++b')))
          ...
          , Just dep <- [lookupComponentInstanceId index ipid]
          ]
