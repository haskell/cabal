...
--       Function defaultPackageDesc isn't using it internally.
  ...
  [
    ...
    . withAllLib $ \Library {exposedModules = ex, libBuildInfo = bi} -> do
       ...
       moduleSrcs <- findModulesFiles (hsSourceDirs bi) pps
                                      (exposed ++ otherModules filteredBi)
  ...
  , fmap concat
    . withAllExe $ \Executable {modulePath = mainPath, buildInfo = bi} -> do
       let filteredBi = filterAutogenBuildInfo pkg_descr bi
       moduleSrcs  <- findModulesFiles (hsSourceDirs bi) pps
                                       (otherModules filteredBi)
       mainSrc <- findMainFile (hsSourceDirs bi) pps mainPath
       return $ (mainSrc:moduleSrcs) ++ cSources filteredBi
                                     ++ jsSources filteredBi
    ...
    . withAllTest $ \t -> do
       ...
       case testInterface t of
         ...
         TestSuiteLibV09 _ m -> do
           ...
           let filteredBi = filterAutogenBuildInfo pkg_descr bi
           moduleSrcs <- findModulesFiles (hsSourceDirs bi) pps
                                          (testModule ++ otherModules filteredBi)
           return $ moduleSrcs ++ cSources filteredBi ++ jsSources filteredBi
...
-- | Given a list of modules removes the ones autogenerated based on the package
-- and build info of the component the modules belong.
filterAutogenModules :: PackageDescription -- ^ The package of this modules
                     -> BuildInfo          -- ^ The build info of this modules
                     -> [ModuleName]       -- ^ The input modules
                     -> [ModuleName]
filterAutogenModules pkg_descr bi modules =
  let pathsModule = autogenPathsModuleName pkg_descr
  in filter
    (\mn ->
          -- TODO: When lower than 1.25 is deprecated every Paths_* module must
          -- appear on autogenModules and this first check can be removed.
          mn /= pathsModule
      &&  not (elem mn (autogenModules bi))
    )
    modules

-- | Given a list of sources removes the ones autogenerated.
filterAutogenSources :: [FilePath] -> [FilePath] -> [FilePath]
filterAutogenSources autogenSrcs = filter (not . (flip elem) autogenSrcs)

-- | Given a build info removes the modules autogenerated based on itself and
-- the package description.
filterAutogenBuildInfo :: PackageDescription -- ^ The package of this modules
                       -> BuildInfo          -- ^ The build info of this modules
