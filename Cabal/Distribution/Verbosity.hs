...
parseVerbosity :: (P.CharParsing m, Monad m) => m (Either Int Verbosity)
parseVerbosity = parseIntVerbosity <|> parseStringVerbosity
  where
    parseIntVerbosity = fmap Left P.integral -- TODO: int to verbosity?
    parseStringVerbosity = fmap Right $ do
        level <- parseVerbosityLevel
        _ <- P.spaces
        extras <- P.sepBy parseExtra P.spaces
...
flagToVerbosity = parsecToReadE ("flagToVerbosity: " ++) $ do
    e <- parseVerbosity
    case e of
        Left i -> case intToVerbosity i of
           Just v  -> return v
           Nothing -> fail ("Bad verbosity: " ++ show i ++
