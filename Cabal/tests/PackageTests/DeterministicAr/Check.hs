...
import Distribution.Simple.LocalBuildInfo (LocalBuildInfo, compiler, localComponentInstanceId)
...
checkMetadata lbi dir = withBinaryFile path ReadMode $ \ h -> do
  ...
  where
    path = dir </> "lib" ++ getHSLibraryName (localComponentInstanceId lbi) ++ ".a"
