...
Foreign libraries are system libraries intended to be linked against programs written in C or other &ldquo;foreign&rdquo; languages. They come in two primary flavours: _dynamic_ libraries (`.so` files on Linux, `.dylib` files on OSX, `.dll` files on Windows, etc.) are linked against executables when the executable is run (or even lazily during execution), while _static_ libraries (`.a` files on Linux/OSX, `.lib` files on Windows) get linked against the executable at compile time.
...
to way of finding out where a platform library got installed (other than by
searching the `lib/` directory). Instead, we install foreign libraries in
`~/.cabal/lib`, must like we install executables in `~/.cabal/bin`. 
