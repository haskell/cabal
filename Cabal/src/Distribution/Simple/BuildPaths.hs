...
haddockLibraryDirName
  ...
  -> Library
  -> FilePath
haddockLibraryDirName ForDevelopment pkg_descr lib =
  case libName lib of
...
haddockLibraryDirName ForHackage pkg_descr _lib = (++ "-docs") . prettyShow . packageId $ pkg_descr
...
-- | A name of a (sub)library directory.
--
-- This is different from `haddockPackageLibraryName` as it contains `:` which
-- is not a valid char in file path on Windows.
haddockPackageLibraryDirName :: PackageDescription -> Library -> String
haddockPackageLibraryDirName pkg_descr lib =
  case libName lib of
    LSubLibName sublib_name ->
      prettyShow (packageName pkg_descr) ++ "-" ++ prettyShow sublib_name
