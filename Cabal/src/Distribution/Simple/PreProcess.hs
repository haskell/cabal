...
-- @mtl@ versions before 2.3.0 re-exported @Control.Monad@ from
-- @Control.Monad.Writer@ (see https://github.com/haskell/mtl/blob/master/docs/Mtl-2.3.x-Migration.md),
-- triggering a redundant import warning here; from @mtl-2.3.0@ onwards, this
-- re-export no longer exists, so we need to explicitly import @Control.Monad@
-- here.
      ...
      when (not forSDist || forSDist && platformIndependent pp) $ do
        ...
        void . execWriterT $
          rerunIfChanged'
            ...
            sequence
            [
              ( newFileMonitor (i buildLoc </> srcStem <.> ext <> "-cache")
              , key
              , recompile
              )
            ]
...
ppUnlit =
  PreProcessor
    ...
    , getPreProcessor = \_ -> do
        ...
        return (runPreProcessor, PreProcessorBuiltin)
    }
