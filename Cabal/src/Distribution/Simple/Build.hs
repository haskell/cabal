...
buildComponent verbosity numJobs pkg_descr lbi suffixes
    ...
    preprocessComponent pkg_descr comp lbi clbi False verbosity suffixes
    extras <- preprocessExtras verbosity comp lbi
    generatedExtras <- generateCode (testCodeGenerators test) (testName test) pkg_descr (testBuildInfo test) lbi clbi verbosity
    setupMessage' verbosity "Building" (packageId pkg_descr)
      (componentLocalName clbi) (maybeComponentInstantiatedWith clbi)
    let ebi = buildInfo exe
        exe' = exe { buildInfo = addExtraOtherModules (addExtraCSources ebi extras) generatedExtras }
    buildExe verbosity numJobs pkg_descr lbi exe' clbi
    return Nothing
...
buildComponent verbosity numJobs pkg_descr lbi0 suffixes
    ...
    setupMessage' verbosity "Building" (packageId pkg_descr)
      (componentLocalName clbi) (maybeComponentInstantiatedWith clbi)
    let libbi = libBuildInfo lib
        lib' = lib { libBuildInfo = addExtraOtherModules libbi generatedExtras }
...
-- | Add extra HS sources generated by preprocessing to build
-- information.
