...
chattyTry verbosity desc action =
  catchIO action $ \exception ->
    hPutStrLn (verbosityStderrHandle verbosity) $
...
rawSystemIOWithEnvAndAction verbosity path args mcwd menv action inp out err =
  withFrozenCallStack $ do
    ...
    -- However, the 'process' documentation for 'createProcess'/'withCreateProcess'
    -- states:
    --
    ...
    --
    -- This means we should not use "UseHandle (verbosityStdoutHandle)", as this
    -- will close the verbosity handle in the parent thread, preventing us from
    -- continuing logging. To solve this, we duplicate the handles first.
    outHandle <-
      case out of
        Just h -> pure h
        Nothing -> hDuplicate (verbosityStdoutHandle verbosity)
    errHandle <-
      case err of
        Just h -> return h
        Nothing -> hDuplicate (verbosityStderrHandle verbosity)
