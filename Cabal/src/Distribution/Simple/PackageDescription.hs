...
readAndParseFile
  :: (BS.ByteString -> ParseResult CabalParserSource a)
...
parseString parser verbosity name bs = do
  ...
  traverse_ (warn verbosity . showPWarningWithSource . fmap renderCabalParserSource) (flattenDups verbosity warnings)
  case result of
    Right x -> return x
    Left (_, errors) -> do
      traverse_ (warn verbosity . showPErrorWithSource . fmap renderCabalParserSource) errors
...
flattenDups verbosity ws
  ...
  where
    ...
    flatCount (PWarningWithSource source (PWarning t pos w) : xs) =
      [ PWarningWithSource source (PWarning
          t
          pos
