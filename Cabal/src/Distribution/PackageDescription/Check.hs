...
checkExecutable pkg exe =
  ...
  , check (not (null (modulePath exe))
      ...
      PackageBuildImpossible $
           "The 'main-is' field must specify a '.hs', '.lhs' or '.lhs.*' file "
  ...
  , checkSpecVersion pkg CabalSpecV1_18
        ...
        && not (fileExtensionHaskell $ modulePath exe)) $
      PackageDistInexcusable $
...
checkTestSuite pkg test =
  ...
  where
    ...
    mainIsNotHsExt = case testInterface test of
      TestSuiteExeV10 _ f -> not $ fileExtensionHaskell f
      _                   -> False
...
checkBenchmark _pkg bm =
  ...
  where
    ...
    mainIsWrongExt = case benchmarkInterface bm of
      BenchmarkExeV10 _ f -> not $ fileExtensionHaskell f
      _                   -> False
...
fileExtensionSupportedLanguage path =
    fileExtensionHaskell path || isC
  where
    isC = isJust (filenameCDialect $ takeExtension path)

fileExtensionHaskell :: FilePath -> Bool
fileExtensionHaskell path =
    isHaskell || isLiterate || isLiterateAlt
  where
    (name, extension) = splitExtension path
    isHaskell     = extension == ".hs"
    isLiterate    = extension == ".lhs"

    -- Penultimate extension may also be .lhs so the last one can indicate
    -- an actual format to unlit code blocks from.
    --
    -- Note: May require passing @-x lhs@ to GHC.
    isLiterateAlt = takeExtension name == ".lhs"
