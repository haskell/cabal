...
import Distribution.PackageDescription.FieldGrammar (CompatDataDir, CompatLicenseFile)
...
import Distribution.Utils.Path (RelativePath, SymbolicPath)
...
reHsString = RENamed "hs-string" impl
  where
...
convert = go id . vacuous
  where
    ...
    go f (REMunch sep r) = RE.Eps RE.\/ r' <> RE.star_ (sep' <> r')
      where
        sep' = go f sep
        r' = go f r
    go f (REMunch1 sep r) = r' <> RE.star_ (sep' <> r')
      where
        sep' = go f sep
        r' = go f r
...
testDescribed
  :: forall a
   . (Arbitrary a, Described a, Typeable a, Eq a, Show a)
  => Proxy a
  -> TestTree
testDescribed _ =
  testGroup
...
instance Described LibVersionInfo where
  describe _ = reDigits <> REOpt (reChar ':' <> reDigits <> REOpt (reChar ':' <> reDigits))
    where
...
instance Described ModuleName where
  describe _ = REMunch1 (reChar '.') component
    where
...
instance Described PackageVersionConstraint where
  describe _ =
    describe (Proxy :: Proxy PackageName)
      <> REUnion
...
instance Described Version where
  describe _ = REMunch1 reDot reDigits
    where
      reDigits =
        REUnion
          [ reChar '0'
