...
import Test.QuickCheck (Arbitrary (..), NonEmptyList (..), NonNegative (..), Property, Testable, arbitrarySizedNatural, choose, counterexample, property, sized, vectorOf, (===), (==>))
...
prop_parse_disp2 vr =
  let b = fmap (prettyShow :: VersionRange -> String) (simpleParsec (prettyShow vr))
   ...
   in counterexample ("Expected: " ++ show a) $
