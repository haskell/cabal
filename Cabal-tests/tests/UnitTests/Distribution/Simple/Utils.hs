...
import System.IO (hClose, hPutStrLn, localeEncoding)
  ...
  | otherwise = do
      ...
      res <- withTempFile tempDir ".hs" $ \filenameHs handleHs -> do
        withTempFile tempDir ".exe" $ \filenameExe handleExe -> do
          ...
          -- Compile
          (resOutput, resErrors, resExitCode) <-
            rawSystemStdInOut
              normal
              ghcPath
              ["-o", filenameExe, filenameHs]
              Nothing
              Nothing
              Nothing
              IODataModeText
      ...
      case res of
        ...
        Left err
          | isDoesNotExistError err -> Exception.throwIO err -- no ghc!
          | otherwise -> return ()
