...
inline int powerAndMod(const int x, int power, const int mod) {
...
int hash(const int intKey, const int hashId, const int salt) {
  assert((intKey + HASH_KEY_MAX_SALT) * (intKey + HASH_KEY_MAX_SALT) <
      HASH_KEY_MAX_INT);
  int primeP = 0;
  int modP = 0;
  assert(hashId >= 0 && hashId <= HASH_KEY_MAX_HASHID);
  if (hashId == 0) {
    primeP = 306133;
    modP = 44879;
  } else if (hashId == 1) {
...
int getEntryLength(const int length) {
  if (length == 1) {
    return 1;
  } else {
...
int getTableLength(const int length) {
  return MAX(2, (length * 4) / 2);
}

void printTable(const struct IntDict* p) {
  printf("---TABLE---\n");
  printf("size: %d\n", p->size);
...
struct IntDict* makeNewTable(const int newTableLength) {
  struct IntDict* result = malloc(sizeof(struct IntDict));
...
void destroyTable(const struct IntDict* p) {
  int i = 0;
  for (i = 0; i < p->tableLength; i += 1) {
...
void add(struct IntDict** pp, const int k, const int v);

void intDictResize(struct IntDict** pp) {
  struct IntDict* p = *pp;
  int newTableLength = getTableLength(p->size);
  struct IntDict* newTable = makeNewTable(newTableLength);
  assert(newTableLength != p->tableLength);
  int i = 0;
  for (i = 0; i < p->tableLength; i += 1) {
    int j = 0;
    for (j = 0; j < getEntryLength(p->table[i].length); j += 1) {
      if (p->table[i].containers[j].exists) {
        add(&newTable, p->table[i].containers[j].key, p->table[i].containers[j].value);
      } else {
        continue;
      }
    }
...
void destroyIntDict(struct IntDict** pp) {
  destroyTable(*pp);
  free(pp);
...
struct IntDict** newIntDict() {
  struct IntDict** pp = malloc(sizeof(struct IntDict*));
  struct IntDict* p = malloc(sizeof(struct IntDict));
  *pp = p;
  p->table = malloc(1);
  p->tableLength = 0;
  p->size = 0;
...
int lookup(const struct IntDict** pp, const int k) {
  const struct IntDict* p = *pp;
  /*printTable(p);*/
  int entryId = hash(k, HASH_KEY_MAX_HASHID, HASH_KEY_TABLE_SALT) % p->tableLength;
  if (p->table[entryId].length == 0) { return 0; }
  int containerId = (hash(k, p->table[entryId].hashId, p->table[entryId].salt) %
      getEntryLength(p->table[entryId].length));
  struct Container* pContainer = &(p->table[entryId].containers[containerId]);
  if ((*pContainer).exists && (*pContainer).key == k) {
    return (*pContainer).value;
  } else {
    return 0;
  }
}

int find(const struct IntDict** pp, const int k) {
  const struct IntDict* p = *pp;
  int entryId = hash(k, HASH_KEY_MAX_HASHID, HASH_KEY_TABLE_SALT) % p->tableLength;
  if (p->table[entryId].length == 0) { return 0; }
  int containerId = (hash(k, p->table[entryId].hashId, p->table[entryId].salt) %
      getEntryLength(p->table[entryId].length));
  if (p->table[entryId].containers[containerId].key == k) {
    return 1;
  } else {
    return 0;
  }
...
void add(struct IntDict** pp, const int k, const int v) {
  struct IntDict* p = *pp;
  int kExists = find(pp, k);

  int entryId = hash(k, HASH_KEY_MAX_HASHID, HASH_KEY_TABLE_SALT) % p->tableLength;
  int newEntryLength = getEntryLength(p->table[entryId].length + (kExists ? 0 : 1));
  ...
  while (1) {
    ...
    int newContainerId = hash(k, entry->hashId, entry->salt) % newEntryLength;
    newContainers[newContainerId].exists = 1;
    newContainers[newContainerId].key = k;
    ...
    for (i = 0; i < getEntryLength(entry->length); i += 1) {
      ...
      if (!entry->containers[i].exists) {
        ...
        continue;
      } else if (entry->containers[i].key == k) {
  ...
  if (getTableLength(p->size) >= 2 * (p->tableLength)) {
    intDictResize(pp);   
  }
...
void intDictRemove(struct IntDict** pp, const int k) {
  struct IntDict* p = *pp;
  if (!find(pp, k)) {
    return;
  }
  ...
  while (1) {
    newContainers = malloc(newEntryLength * sizeof(struct Container));
    // Move new value into new container list
    int oldContainerId = hash(k, entry->hashId, entry->salt) % entry->length;
    entry->containers[oldContainerId].exists = 0; // block out container to be removed;
    ...
    entry->containers[oldContainerId].exists = 0; // remove blocked out container
    if (noCollisions) { break; }
  }
  free(entry->containers);
  entry->containers = newContainers;
  ...
  if (getTableLength(p->size) < p->tableLength / 4) {
    intDictResize(pp);
  }
...
int size(const struct IntDict** pp) {
  const struct IntDict* p = *pp;
  return p->size;
}
