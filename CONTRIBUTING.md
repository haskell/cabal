...
3. **Gate the field in the grammar.** In
   ...
   field with `availableSince <Constructor> <default>`.

   The second argument has the field's own type (see
   `Distribution.FieldGrammar.Class`): it is the value the field takes in a
   package declaring an older `cabal-version`, i.e. the behaviour that was in
   effect before the field existed. For list-valued fields that is `[]`, for
   optional fields `Nothing`, and for enumerations the constructor matching
   the old behaviour — e.g. `availableSince CabalSpecV3_0
   LibraryVisibilityPrivate` and `availableSince CabalSpecV2_0
   ExecutablePublic`.

   > ⚠️ Use the **same literal constructor you set `cabalSpecLatest` to in
   > step 2** (e.g. `availableSince CabalSpecV3_18 []`) — **not** the name
