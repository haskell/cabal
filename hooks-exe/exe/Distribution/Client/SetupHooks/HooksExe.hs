...
hooksMain setupHooks = do
  ...
  case args of
    ...
    inputFdRef : outputFdRef : hooksExeArgs -> do
      ...
      case hReadMb of
        Nothing ->
          dieWithException hooksExeVerbosity $
          ...
          case hWriteMb of
            ...
            Just hWrite ->
              ...
              case hooksExeArgs of
                ...
                _ -> dieWithException hooksExeVerbosity NoHookType
    _ -> dieWithException hooksExeVerbosity $
           NoHandle Nothing
  where
    allHookHandlers =
      [ (nm, action)
     ...
     in HookHandler hookName $ \h _ ->
          ...
          runHookHandle h hookName $ \(ruleId, ruleDeps) ->
            case runRuleDynDepsCmd ruleDeps of
              Nothing -> dieWithException hooksExeVerbosity $ BadHooksExeArgs hookName $ NoDynDepsCmd ruleId
