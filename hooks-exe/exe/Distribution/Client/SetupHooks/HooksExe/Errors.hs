...
import Distribution.Verbosity
  ( Verbosity, mkVerbosity, defaultVerbosityHandles )
...
hooksExeVerbosity = mkVerbosity defaultVerbosityHandles Verbosity.normal
  -- NB: the hooks executable is always invoked as a separate process, never as
  -- a library function, so we can use stdout/stderr for verbosity and redirect
  -- them using the System.Process API.
