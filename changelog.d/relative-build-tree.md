---
synopsis: Make build tree relocatable with `--relative-build-tree`
...
When using the flag `--relative-build-tree` (or `relative-build-tree: True` configuration in a project file), all project-local paths in the build tree (e.g. `dist-newstyle`) are relativized to the build tree root.
