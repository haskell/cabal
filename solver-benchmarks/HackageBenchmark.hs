...
import Statistics.Sample (geometricMean, mean, stdDev)
      ...
      then do
        ...
        if isSignificantResult result1 result2
          ...
          then
            ...
            when (argPrintTrials || argPrintSkippedPackages) $
              putStrLn $
...
runCabal timeoutSeconds cabalUnderTest cabal flags = do
  ...
  let cabalDir =
        tmpDir </> "solver-benchmarks-workdir" </> case cabalUnderTest of
