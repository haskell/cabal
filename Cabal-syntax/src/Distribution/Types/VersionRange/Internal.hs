...
prettyVersionRange t0 vr = snd $ prettyVersionRange' t0 0 vr

data PrettyMode = Exact | BestEffort

prettyVersionRange' :: TriviaTree -> Int -> VersionRange -> (PrettyMode, Disp.Doc)
...
versionRangeTriviaParser digitParser csv = expr
  where
    ...
    expr = do
      ...
      (tTerm, t) <- term
      postSpaces <- P.spaces'
      let tTerm' = fromNamedTrivia t [PreTrivia preSpaces, PostTrivia postSpaces] <> tTerm
      ( do
          _ <- P.string "||"
          checkOp
          preSpaces' <- P.spaces'
          ...
          let tExpr' =
                markTriviaTree
                  eUnion
                  ( fromNamedTrivia e [PreTrivia preSpaces']
                      <> tExpr
                      <> tTerm'
                  )
          return (tExpr', eUnion)
    ...
    term = do
      (tFact, f) <- factor
      postSpaces <- P.spaces'
      let tFact' = fromNamedTrivia f [PostTrivia postSpaces] <> tFact
      ( do
          _ <- P.string "&&"
          checkOp
          preSpaces' <- P.spaces'
          ...
          let tTerm' =
                markTriviaTree
                  tInter
                  ( fromNamedTrivia t [PreTrivia preSpaces']
                      <> tTerm
                      <> tFact'
                  )
          return (tTerm', tInter)
      ...
      case op of
        ...
        "==" -> do
          ...
          ( do
              ...
              let tvr = markTriviaTree vr $ fromNamedTrivia v [PreTrivia preSpaces]
              pure $ (tvr, vr)
        ...
        _ -> do
          ...
          let withTVer ver =
                let t = fromNamedTrivia v [PreTrivia preSpaces]
                 in (markTriviaTree ver t, ver)
          case op of
