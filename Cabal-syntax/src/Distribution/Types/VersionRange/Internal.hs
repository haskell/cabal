...
type VersionRange = VersionRangeWith Abst
type VersionRangeAnn = VersionRangeWith Conc

-- | Dependending on whether the data is a leaf, we annotate differently
type family Modify (m :: ParsingPhase) (a :: Type) where
  Modify Abst a = a
  Modify Conc Version = (Trivia SurroundingText, VersionAnn)
  Modify Conc (VersionRangeWith Conc) = (Trivia SurroundingText, VersionRangeWith Conc)

data VersionRangeWith (m :: ParsingPhase)
...
annVersionRange :: VersionRange -> VersionRangeAnn
annVersionRange (ThisVersion v) = ThisVersion (mempty, Ann mempty v)
annVersionRange (LaterVersion v) = LaterVersion (mempty, Ann mempty v)
annVersionRange (OrLaterVersion v) = OrLaterVersion (mempty, Ann mempty v)
annVersionRange (EarlierVersion v) = EarlierVersion (mempty, Ann mempty v)
annVersionRange (OrEarlierVersion v) = OrEarlierVersion (mempty, Ann mempty v)
annVersionRange (MajorBoundVersion v) = MajorBoundVersion (mempty, Ann mempty v)
annVersionRange (UnionVersionRanges a b) = UnionVersionRanges (mempty, annVersionRange a) (mempty, annVersionRange b)
annVersionRange (IntersectVersionRanges a b) = IntersectVersionRanges (mempty, annVersionRange a) (mempty, annVersionRange b)
...
anyVersionAnnInserted :: VersionRangeAnn
anyVersionAnnInserted = OrLaterVersion (IsInserted, Ann mempty $ mkVersion [0])
...
versionRangeAnnParser digitParser csv = expr
  where
      ...
      case op of
        ...
        "==" -> do
          ...
          ( do
              ...
              <|> ( verSet >>= \versionAnns -> do
                      let versions :: NonEmpty Version = fmap unAnn versionAnns
                      vRange :: VersionRange <- verSet' thisVersion versions
                      let vRangeAnn :: VersionRangeAnn = annVersionRange vRange
                      pure vRangeAnn
                  )
            )
        "^>=" -> do
          ...
          ( do
              ...
              <|> ( verSet >>= \versionAnns -> do
                      let versions :: NonEmpty Version = fmap unAnn versionAnns
                      vRange :: VersionRange <- verSet' majorBoundVersion versions
                      let vRangeAnn :: VersionRangeAnn = annVersionRange vRange
                      pure vRangeAnn
                  )
            )
        ...
        then pure $ majorBoundVersionAnn v
          ...
          fail $
            unwords
              ...
              , "Alternatively, if broader compatibility is important then use:"
              , prettyShow $ eliminateMajorBoundSyntax $ majorBoundVersion $ unAnn $ snd v
              ]
      where
    ...
    verSet' :: (t -> VersionRange) -> NonEmpty t -> m VersionRange
    verSet' op vs =
      if csv >= CabalSpecV3_0
        then pure $ foldr1 (\x y -> unionVersionRanges x y) (fmap op vs)
        else
          fail $
            unwords
              ...
              , prettyShow (foldr1 (\x y -> unionVersionRanges x y) (fmap op vs))
              ]

    -- TODO(leana8959): register this trivia
    verSet :: m (NonEmpty VersionAnn)
    verSet = do
      _ <- P.char '{'
      P.spaces
