...
type HasBuildInfo = HasBuildInfoWith Abst
type HasBuildInfoAnn = HasBuildInfoWith Conc

class HasBuildInfoWith mod a | a -> mod where
  buildInfo :: Lens' a (BuildInfoWith mod)

  buildable :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AnnotateWith Positions mod Bool))
  buildable = buildInfo @mod . buildable @mod

  buildTools :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod LegacyExeDependency)]))
  buildTools = buildInfo @mod . buildTools @mod

  buildToolDepends :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod ExeDependency)]))
  buildToolDepends = buildInfo @mod . buildToolDepends @mod

  cppOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  cppOptions = buildInfo @mod . cppOptions @mod

  asmOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  asmOptions = buildInfo @mod . asmOptions @mod

  cmmOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  cmmOptions = buildInfo @mod . cmmOptions @mod

  ccOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  ccOptions = buildInfo @mod . ccOptions @mod

  cxxOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  cxxOptions = buildInfo @mod . cxxOptions @mod

  jsppOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  jsppOptions = buildInfo @mod . jsppOptions @mod

  ldOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  ldOptions = buildInfo @mod . ldOptions @mod

  hsc2hsOptions :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod String)]))
  hsc2hsOptions = buildInfo @mod . hsc2hsOptions @mod

  pkgconfigDepends :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod PkgconfigDependency)]))
  pkgconfigDepends = buildInfo @mod . pkgconfigDepends @mod

  frameworks :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (RelativePath Framework File))]))
  frameworks = buildInfo @mod . frameworks @mod

  extraFrameworkDirs :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (SymbolicPath Pkg (Dir Framework)))]))
  extraFrameworkDirs = buildInfo @mod . extraFrameworkDirs @mod

  asmSources :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (SymbolicPath Pkg File))]))
  asmSources = buildInfo @mod . asmSources @mod

  cmmSources :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (SymbolicPath Pkg File))]))
  cmmSources = buildInfo @mod . cmmSources @mod

  cSources :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (SymbolicPath Pkg File))]))
  cSources = buildInfo @mod . cSources @mod

  cxxSources :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (SymbolicPath Pkg File))]))
  cxxSources = buildInfo @mod . cxxSources @mod

  jsSources :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod (SymbolicPath Pkg File))]))
  ...
  otherModules :: HasBuildInfoWith mod (BuildInfoWith mod) => Lens' a (PreserveGrouping mod (AttachPositions mod [AttachPosition mod (Annotate mod ModuleName)]))
...
instance HasBuildInfoWith Abst (BuildInfoWith Abst) where
  buildInfo = id
  {-# INLINE buildInfo #-}
...
type HasBuildInfos = HasBuildInfoWith Abst
type HasBuildInfosAnn = HasBuildInfoWith Conc

instance HasBuildInfoWith Conc (BuildInfoWith Conc) where
  buildInfo = id
  {-# INLINE buildInfo #-}
