...
data PRContext src = PRContext
  { prContextSource :: PSource src -- ^ The file we are parsing, if known
  }
...
runParseResult pr = unPR pr emptyPRState initialCtx failure success
  where
    ...
    failure (PRState warns [] v) = (warns, Left (v, PErrorWithSource PUnknownSource (PError Nothing zeroPos "panic") :| []))
...
parseFailure pos msg = PR $ \(PRState warns errs v) ctx _failure success ->
  success (PRState warns (PErrorWithSource (prContextSource ctx) (PError Nothing pos msg) : errs) v) ()
...
parseFatalFailure pos msg = PR $ \(PRState warns errs v) ctx failure _success ->
  failure (PRState warns (PErrorWithSource (prContextSource ctx) (PError Nothing pos msg) : errs) v)
...
parseFatalFailure' = PR pr
  where
    ...
    err = PErrorWithSource PUnknownSource (PError Nothing zeroPos "Unknown fatal error")
