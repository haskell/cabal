...
-- TODO(leana8959): Change this with relative monad in the future
type EpField = FieldConcrete Position (WithComments Position)

-- commented for now, for the Field datatype refactor
...
type MatchSection ann = Name ann -> [SectionArg ann] -> [FieldConcrete Position ann] -> Bool
...
addSection ac name args nameCmts inner = Edit $ \spec ->
  let modifySectionFields (SectionConcrete n as fs) = SectionConcrete n as <$> runEdit inner spec fs
...
mkSectionAt name sargs nameCmts pos =
  let
    ...
    newSection = SectionConcrete nameWithAnn sargsWithAnn []
   in
    newSection
...
mkFieldAt name nameCmts fls flsCmts pos =
  let
    ...
    field = FieldConcrete colonPos nameWithAnn flsWithAnn
    totalHeight =
...
removeField rc match = Edit $ \_ -> case rc of
  ...
  where
    p (FieldConcrete _ name fls) = not (match name fls)
    p _ = True
...
modifyField mc match modifyFieldLines = Edit $ \spec ->
  let doModify (FieldConcrete colonPos fname fls) | match fname fls = FieldConcrete colonPos fname <$> fls'
        where
...
modifySection mc match modifySectionArgs modifyFields = Edit $ \spec ->
  let doModify (SectionConcrete sname sargs fs)
        | match sname sargs fs =
            let sargs' = modifySectionArgs spec sargs
             ...
             in -- TODO(leana8959): handle the case where section args' comments
                ...
                SectionConcrete sname <$> sargs' <*> fs'
      doModify x = EditUnchanged x
...
fieldsRowRange :: (L.HasPosition ann, L.HasPosition pos) => NonEmpty (FieldConcrete pos ann) -> (Int, Int)
...
fieldRowRange :: (L.HasPosition pos, L.HasPosition ann) => FieldConcrete pos ann -> (Int, Int)
fieldRowRange (FieldConcrete _colonPos fname fls) =
...
fieldRowRange (SectionConcrete sname _sargs fs) =
...
afterFieldEndPosition :: (L.HasPosition ann, L.HasPosition pos) => FieldConcrete pos ann -> Position
afterFieldEndPosition f =
  let (_, endRow) = fieldRowRange f
   in Position (endRow + 1 {- next line -}) 1

offsetFieldRow :: (L.HasPosition ann, L.HasPosition pos) => Int -> FieldConcrete pos ann -> FieldConcrete pos ann
offsetFieldRow n = \case
  (FieldConcrete colonPos fname fls) -> FieldConcrete (incrementRowN colonPos) (fmap incrementRowN fname) (map (fmap incrementRowN) fls)
  (SectionConcrete sname sargs fs) -> SectionConcrete (fmap incrementRowN sname) (map (fmap incrementRowN) sargs) (map (fmap incrementRowN) fs)
  where
