...
--
-- element ::= '\n' name elementInLayout
--           |      name ElemBraces
element :: IndentLevel -> Parser Field
element ilevel st
  case getToken st of
    TokIndent ilevel' | ilevel' >= ilevel ->
      case getToken st' of
        TokName ->
          elementInLayoutContext (incIndentLevel ilevel') name st''
        tok -> parseError "element" tok

    TokName name -> elementInNonLayoutContext name st'
    tok          -> parseError "element" tok
