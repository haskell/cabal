...
module Distribution.Solver.Modular (modularResolver, SolverConfig (..), PruneAfterFirstSuccess (..)) where
...
modularResolver sc (Platform arch os) cinfo iidx sidx pkgConfigDB pprefs pcs pns =
  uncurry postprocess
    <$> solve' sc cinfo idx pkgConfigDB pprefs gcs pns -- convert install plan
  where
...
tryToMinimizeConflictSet runSolver sc cs cm =
  foldl
    (\r v -> retryNoSolution r $ tryToRemoveOneVar v)
