...
import Control.Monad (foldM, liftM2, mzero)
...
validate = go
  where
    ...
    goP qpn@(Q _pp pn) (POption i _) r = do
      ...
      let mnppa =
            extend extSupported langSupported pkgPresent newactives
...
merge (MergedDepFixed comp1 vs1 i1) (PkgDep vs2 (PkgComponent p comp2) ci@(Fixed i2))
  ...
  | otherwise =
      Left
        ( (CS.union `on` dependencyReasonToConflictSet) vs1 vs2
        ,
...
merge (MergedDepFixed comp1 vs1 i@(I v _)) (PkgDep vs2 (PkgComponent p comp2) ci@(Constrained vr))
  ...
  | otherwise =
      Left
        ( createConflictSetForVersionConflict p v vs1 vr vs2
        ,
...
merge (MergedDepConstrained vrOrigins) (PkgDep vs2 (PkgComponent p comp2) ci@(Fixed i@(I v _))) =
  ...
  where
    ...
    go ((vr, comp1, vs1) : vros)
      ...
      | otherwise =
          Left
            ( createConflictSetForVersionConflict p v vs2 vr vs1
            ,
...
createConflictSetForVersionConflict
  :: QPN
  -> Ver
  -> DependencyReason QPN
  -> VR
...
createConflictSetForVersionConflict
  ...
  versionRangeDR@(DependencyReason p2 _ _) =
    let hasFlagsOrStanzas (DependencyReason _ fs ss) = not (M.null fs) || not (S.null ss)
     in -- The solver currently only optimizes the case where there is a conflict
...
validateTree cinfo idx pkgConfigDb t =
  runValidate
    (validate t)
    VS
      { supportedExt =
