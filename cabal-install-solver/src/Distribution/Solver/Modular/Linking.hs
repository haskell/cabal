...
makeCanonical lg qpn@(Q pp _) i =
  case lgCanon lg of
    ...
    Just _ ->
      conflict
        ( CS.insert (P qpn) (lgConflictSet lg)
        , "cannot make "
            ++ showQPN qpn
            ++ " canonical member of "
            ++ showLinkGroup lg
...
updateLinkGroup lg = do
  ...
  modify $ \vs ->
    vs
      { vsLinks =
          M.fromList (map aux (S.toList (lgMembers lg)))
            `M.union` vsLinks vs
      }
  where
...
verifyFlag' (FN pn fn) lg = do
  ...
  if allEqual (catMaybes vals) -- We ignore not-yet assigned flags
    then return ()
      ...
      conflict
        ( CS.fromList (map F flags) `CS.union` lgConflictSet lg
...
verifyStanza' (SN pn sn) lg = do
  ...
  if allEqual (catMaybes vals) -- We ignore not-yet assigned stanzas
    then return ()
      ...
      conflict
        ( CS.fromList (map S stanzas) `CS.union` lgConflictSet lg
