...
module Distribution.Solver.Modular.Message (
    ...
    -- Requird by the doctests, but not used elsewhere so export this
...
displayMessage (LogSkipMany qsn b cs) = "skipping: " ++ showOptions qsn b ++ " " ++ showConflicts cs
...
--
displayMessage (LogRejectMany qpn is c fr) = "rejecting: " ++ showQPNConflictSet qpn (reverse is) ++ showFR c fr
...
showQPNConflictSet qpn popts =
  case popts of
    [] -> ""
    [x] -> showQPNPOpt qpn x
    (x:xs) -> showQPNPOpt qpn x ++ ", " ++ L.intercalate ", " (map (\(POption i _) -> showI i) xs)
