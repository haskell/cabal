  ...
  where
    ...
    conflictMsg =
      if S.member CS.OtherConflict conflicts
        then -- This case shouldn't happen, because an unknown conflict should not
        -- cause a version to be skipped.
          ...
          let mergedConflicts =
           ...
           in if L.null mergedConflicts
                then -- This case shouldn't happen unless backjumping is turned off.
                  "none"
          ...
          catMaybes
            [ case () of
                ()
                  ...
                  | not $ L.null (versionConstraintConflict mergedConflict) ->
                      Just $
                        "excludes '"
                          ++ showQPN qpn
                          ++ "' "
...
showConflictingDep (ConflictingDep dr (PkgComponent qpn comp) ci) =
  let DependencyReason qpn' _ _ = dr
   ...
   in case ci of
        Fixed i ->
          (if qpn /= qpn' then showDependencyReason dr ++ " => " else "")
            ++ showQPN qpn
            ++ componentStr
            ++ "=="
            ++ showI i
