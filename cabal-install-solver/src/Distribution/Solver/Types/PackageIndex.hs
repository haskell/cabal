...
fromList pkgs =
  ...
  where
    fixBucket =
      -- out of groups of duplicates, later ones mask earlier ones
...
delete
  ...
  -> PackageIndex pkg
  -> PackageIndex pkg
delete name p (PackageIndex index) =
  mkPackageIndex $
...
lookupPackageId index pkgid =
  case [ pkg | pkg <- lookup index (packageName pkgid), packageId pkg == pkgid
       ] of
    [] -> Nothing
    [pkg] -> Just pkg
...
lookupDependency index name versionRange =
  [ pkg | pkg <- lookup index name, packageName pkg == name, packageVersion pkg `withinRange` versionRange
  ]
