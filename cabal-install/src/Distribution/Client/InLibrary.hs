...
configure
  ...
  cfg =
    ...
    do
      ...
      packageDBs <- (traverse . traverse) (fmap makeSymbolicPath . canonicalizePath) packageDBsCWD
      ...
      setProfiling <- Cabal.configureProfiling verbosity cfg compil
      setCoverage <- Cabal.configureCoverage verbosity cfg compil

      let lbc1 = lbc0{LBC.withBuildOptions = setCoverage $ setProfiling $ LBC.withBuildOptions lbc0}
      ...
      lbc2 <- case preConfPackageHook confHooks of
        Nothing -> return lbc1
        Just hk -> Cabal.runPreConfPackageHook cfg compil plat lbc1 hk
      ...
      (lbc3, pbd2) <-
        Cabal.configurePackage
          ...
          plat
          packageDBs
      for_ (postConfPackageHook confHooks) $ Cabal.runPostConfPackageHook lbc3 pbd2
        ...
        applyComponentDiffs
          ...
          ( \comp ->
              if wantComponent compRequested comp
                then traverse (Cabal.runPreConfComponentHook lbc3 pbd2 comp) $ preConfComponentHook confHooks
                else return Nothing
          )
          pkg_descr2
      ...
      let progdb = LBC.withPrograms lbc3
