    ...
    , commandDescription = Just $ \_ ->
        wrapText $
          "Query for configuration and project information such as project GHC.\n"
            <> "This command always requires the flag '--output-format="
...
pathAction flags@NixStyleFlags{extraFlags = pathFlags', ..} cliTargetStrings globalFlags = withContextAndSelectors AcceptNoTargets Nothing flags [] globalFlags OtherCommand $ \_ baseCtx _ -> do
  ...
  when (NoFlag == pathOutputFormat pathFlags) $
    dieWithException verbosity (CmdPathRequiresOutputFormat (map pathOutputFormatString [minBound .. maxBound]))
  ...
  let output = case fromFlagOrDefault JSON (pathOutputFormat pathFlags) of
        JSON ->
