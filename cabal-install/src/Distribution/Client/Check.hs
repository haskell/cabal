...
import Distribution.Client.Utils.Parsec (renderParseError, renderCabalParserSourceMsgs)
...
readGenericPackageDescriptionCheck :: Verbosity -> FilePath -> IO ([PWarningWithSource CabalParserSource], GenericPackageDescription)
readGenericPackageDescriptionCheck verbosity fpath = do
  ...
  case result of
    Left (_, errors) -> do
      traverse_ (warn verbosity . showPWarningWithSource . fmap renderCabalParserSource) warnings
      hPutStr stderr $ renderParseError renderCabalParserSourceMsgs errors warnings -- TODO: MP
