...
setupWrapper verbosity options mpkg cmd getCommonFlags getFlags getExtraArgs wrapperArgs = do
  ...
  let notInLibraryMethod :: kind ~ GeneralSetup => IO (SetupRunnerRes setupSpec)
      notInLibraryMethod =
...
compileSetup verbosity platform pkgId bt opts ver mbCompId forceCompile = do
  case bt of
    Hooks -> compileHooksScript verbosity platform pkgId opts ver mbCompId forceCompile
...
-- | Invokes the compiler to compile the setup.hs / hooks.hs
compileSetupX
  :: String
