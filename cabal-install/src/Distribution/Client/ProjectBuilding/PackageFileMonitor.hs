  ...
  where
    ...
    elab_config =
      elab
        ...
        , -- Whether a relative build tree was requested only affects how paths 
          -- are recorded in generated metadata, not what is compiled, so null it here.
          elabBuildOptions = (elabBuildOptions elab){LBC.relativeBuildTree = False}
        }
