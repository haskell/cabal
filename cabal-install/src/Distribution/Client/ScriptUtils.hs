  ...
  where
    ...
    scriptOrError script err = do
      ...
      if exists then do
        ...
        (compiler, Platform arch os, _) <- runRebuild (distProjectRootDirectory . distDirLayout $ ctx) $ configureCompiler verbosity (distDirLayout ctx) (fst $ ignoreConditions projectCfgSkeleton)
