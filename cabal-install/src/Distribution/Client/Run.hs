...
splitRunArgs verbosity lbi args =
  case whichExecutable of -- Either err (wasManuallyChosen, exe, paramsRest)
    Left err -> do
      ...
      dieWithExceptionCabalInstall verbosity $ SplitRunArgs err
    ...
    if withDynExe lbi
      then do
        ...
        clbi <- case componentNameTargets' pkg_descr lbi (CExeName (exeName exe)) of
          ...
          [] -> dieWithExceptionCabalInstall verbosity CouldNotFindExecutable
          _ -> dieWithExceptionCabalInstall verbosity FoundMultipleMatchingExes
