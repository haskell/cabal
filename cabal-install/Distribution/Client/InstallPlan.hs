...
import Distribution.Package
         ...
         , HasInstalledUnitId(..), InstalledUnitId(..) )
import Distribution.Client.Types
         ( BuildSuccess, BuildFailure
         , PackageFixedDeps(..), ConfiguredPackage
         , GenericReadyPackage(..), fakeInstalledUnitId )
...
type PlanPackage = GenericPlanPackage
                   InstalledUnitInfo ConfiguredPackage
                   BuildSuccess BuildFailure
...
instance (PackageFixedDeps srcpkg,
          PackageFixedDeps ipkg, HasInstalledUnitId ipkg) =>
...
instance (HasInstalledUnitId ipkg, HasInstalledUnitId srcpkg) =>
         HasInstalledUnitId
...
data GenericInstallPlan ipkg srcpkg iresult ifailure = GenericInstallPlan {
    ...
    planVertexOf   :: InstalledUnitId -> Graph.Vertex,
    planIndepGoals :: Bool
  }
...
invariant :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
              HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
...
showPlanIndex :: (HasInstalledUnitId ipkg, HasInstalledUnitId srcpkg)
...
showInstallPlan :: (HasInstalledUnitId ipkg, HasInstalledUnitId srcpkg)
...
--
new :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
        HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
    => Bool
...
new indepGoals index =
  ...
  let isPreExisting (PreExisting _) = True
      ...
      fakeMap = Map.fromList
              . map (\p -> (fakeInstalledUnitId (packageId p)
                           ,installedUnitId p))
...
--
remove :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
           HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
...
ready plan = assert check readyPackages
  where
    ...
    isInstalledDep :: InstalledUnitId -> Maybe ipkg
    isInstalledDep pkgid =
      ...
      case PlanIndex.fakeLookupInstalledUnitId
...
--
processing :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
               HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
           => [GenericReadyPackage srcpkg ipkg]
           -> GenericInstallPlan ipkg srcpkg iresult ifailure
           -> GenericInstallPlan ipkg srcpkg iresult ifailure
...
--
completed :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
              HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
          => InstalledUnitId
          -> Maybe ipkg -> iresult
          -> GenericInstallPlan ipkg srcpkg iresult ifailure
  ...
  where
    plan'     = plan {
                  ...
                  planIndex = PackageIndex.insert installed
                            . PackageIndex.deleteInstalledUnitId pkgid
                            $ planIndex plan
                }
...
-- state.
--
failed :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
           HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
       => InstalledUnitId         -- ^ The id of the package that failed to install
...
lookupProcessingPackage plan pkgid =
  ...
  -- planFakeMap
  case PackageIndex.lookupInstalledUnitId (planIndex plan) pkgid of
...
--
valid :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
          HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
      => FakeMap -> Bool
      -> PlanIndex ipkg srcpkg iresult ifailure
      -> Bool
...
--
problems :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
             HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
         => FakeMap -> Bool
         -> PlanIndex ipkg srcpkg iresult ifailure
...
problems fakeMap indepGoals index =
     ...
     [ PackageMissingDeps pkg
       (catMaybes
        (map
         (fmap packageId . PlanIndex.fakeLookupInstalledUnitId fakeMap index)
         missingDeps))
  ...
  ++ [ PackageStateInvalid pkg pkg'
     ...
     , Just pkg' <- map (PlanIndex.fakeLookupInstalledUnitId fakeMap index)
...
--
acyclic :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
            HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
...
--
closed :: (HasInstalledUnitId ipkg, PackageFixedDeps ipkg,
...
--
consistent :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
               HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
           => FakeMap -> PlanIndex ipkg srcpkg iresult ifailure -> Bool
...
dependencyClosure :: (HasInstalledUnitId ipkg,   PackageFixedDeps ipkg,
                      HasInstalledUnitId srcpkg, PackageFixedDeps srcpkg)
                  => GenericInstallPlan ipkg srcpkg iresult ifailure
                  -> [PackageIdentifier]
...
dependencyClosure installPlan pids =
    PlanIndex.dependencyClosure
      ...
      (map (resolveFakeId . fakeInstalledUnitId) pids)
  where
