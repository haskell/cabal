...
import Distribution.Package
         ...
         , HasPackageKey(..), PackageKey(..) )
import Distribution.Client.Types
         ( BuildSuccess, BuildFailure
         , PackageFixedDeps(..), ConfiguredPackage
         , GenericReadyPackage(..), fakePackageKey )
...
instance (PackageFixedDeps srcpkg,
          PackageFixedDeps ipkg, HasPackageKey ipkg) =>
...
instance (HasPackageKey ipkg, HasPackageKey srcpkg) =>
         HasPackageKey
         (GenericPlanPackage ipkg srcpkg iresult ifailure) where
  packageKey (PreExisting ipkg ) = packageKey ipkg
  packageKey (Configured  spkg)  = packageKey spkg
  packageKey (Processing  rpkg)  = packageKey rpkg
  -- NB: defer to the actual installed package info in this case
  packageKey (Installed _ (Just ipkg) _) = packageKey ipkg
  packageKey (Installed rpkg _        _) = packageKey rpkg
  packageKey (Failed      spkg        _) = packageKey spkg
...
data GenericInstallPlan ipkg srcpkg iresult ifailure = GenericInstallPlan {
    ...
    planVertexOf   :: PackageKey -> Graph.Vertex,
    planIndepGoals :: Bool
  }
...
invariant :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
              HasPackageKey srcpkg, PackageFixedDeps srcpkg)
...
showPlanIndex :: (HasPackageKey ipkg, HasPackageKey srcpkg)
...
showPlanIndex index =
  ...
  where showPlanPackage p =
            showPlanPackageTag p ++ " "
                ++ display (packageId p) ++ " ("
                ++ display (packageKey p) ++ ")"

showInstallPlan :: (HasPackageKey ipkg, HasPackageKey srcpkg)
...
--
new :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
        HasPackageKey srcpkg, PackageFixedDeps srcpkg)
    => Bool
...
new indepGoals index =
  ...
  let isPreExisting (PreExisting _) = True
      ...
      fakeMap = Map.fromList
              . map (\p -> (fakePackageKey (packageId p)
                           ,packageKey p))
...
--
remove :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
           HasPackageKey srcpkg, PackageFixedDeps srcpkg)
...
ready plan = assert check readyPackages
  where
    ...
    isInstalledDep :: PackageKey -> Maybe ipkg
    isInstalledDep pkgid =
      ...
      case PlanIndex.fakeLookupPackageKey
...
--
processing :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
               HasPackageKey srcpkg, PackageFixedDeps srcpkg)
           => [GenericReadyPackage srcpkg ipkg]
           -> GenericInstallPlan ipkg srcpkg iresult ifailure
           -> GenericInstallPlan ipkg srcpkg iresult ifailure
...
--
completed :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
              HasPackageKey srcpkg, PackageFixedDeps srcpkg)
          => PackageKey
          -> Maybe ipkg -> iresult
          -> GenericInstallPlan ipkg srcpkg iresult ifailure
  ...
  where
    plan'     = plan {
                  ...
                  planIndex = PackageIndex.insert installed
                            . PackageIndex.deletePackageKey pkgid
                            $ planIndex plan
                }
    ...
    insert_fake_mapping (Just ipkg) = Map.insert pkgid (packageKey ipkg)
...
-- state.
--
failed :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
           HasPackageKey srcpkg, PackageFixedDeps srcpkg)
       => PackageKey         -- ^ The id of the package that failed to install
...
lookupProcessingPackage plan pkgid =
  ...
  -- planFakeMap
  case PackageIndex.lookupPackageKey (planIndex plan) pkgid of
...
--
valid :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
          HasPackageKey srcpkg, PackageFixedDeps srcpkg)
      => FakeMap -> Bool
      -> PlanIndex ipkg srcpkg iresult ifailure
      -> Bool
...
--
problems :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
             HasPackageKey srcpkg, PackageFixedDeps srcpkg)
         => FakeMap -> Bool
         -> PlanIndex ipkg srcpkg iresult ifailure
...
problems fakeMap indepGoals index =
     ...
     [ PackageMissingDeps pkg
       (catMaybes
        (map
         (fmap packageId . PlanIndex.fakeLookupPackageKey fakeMap index)
         missingDeps))
  ...
  ++ [ PackageStateInvalid pkg pkg'
     ...
     , Just pkg' <- map (PlanIndex.fakeLookupPackageKey fakeMap index)
...
--
acyclic :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
            HasPackageKey srcpkg, PackageFixedDeps srcpkg)
...
--
closed :: (HasPackageKey ipkg, PackageFixedDeps ipkg,
...
--
consistent :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
               HasPackageKey srcpkg, PackageFixedDeps srcpkg)
           => FakeMap -> PlanIndex ipkg srcpkg iresult ifailure -> Bool
...
dependencyClosure :: (HasPackageKey ipkg,   PackageFixedDeps ipkg,
                      HasPackageKey srcpkg, PackageFixedDeps srcpkg)
                  => GenericInstallPlan ipkg srcpkg iresult ifailure
                  -> [PackageIdentifier]
...
dependencyClosure installPlan pids =
    PlanIndex.dependencyClosure
      ...
      (map (resolveFakeId . fakePackageKey) pids)
  where
