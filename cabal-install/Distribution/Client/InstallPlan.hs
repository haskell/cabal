...
import Distribution.Package
         ...
         , HasComponentInstanceId(..), ComponentInstanceId(..) )
import Distribution.Client.Types
         ( BuildSuccess, BuildFailure
         , PackageFixedDeps(..), ConfiguredPackage
         , GenericReadyPackage(..), fakeComponentInstanceId )
...
instance (PackageFixedDeps srcpkg,
          PackageFixedDeps ipkg, HasComponentInstanceId ipkg) =>
...
instance (HasComponentInstanceId ipkg, HasComponentInstanceId srcpkg) =>
         HasComponentInstanceId
         (GenericPlanPackage ipkg srcpkg iresult ifailure) where
  installedSourceHash (PreExisting ipkg ) = installedSourceHash ipkg
  installedSourceHash (Configured  spkg)  = installedSourceHash spkg
  installedSourceHash (Processing  rpkg)  = installedSourceHash rpkg
  -- NB: defer to the actual installed package info in this case
  installedSourceHash (Installed _ (Just ipkg) _) = installedSourceHash ipkg
  installedSourceHash (Installed rpkg _        _) = installedSourceHash rpkg
  installedSourceHash (Failed      spkg        _) = installedSourceHash spkg
...
data GenericInstallPlan ipkg srcpkg iresult ifailure = GenericInstallPlan {
    ...
    planVertexOf   :: ComponentInstanceId -> Graph.Vertex,
    planIndepGoals :: Bool
  }
...
invariant :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
              HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
...
showPlanIndex :: (HasComponentInstanceId ipkg, HasComponentInstanceId srcpkg)
...
showPlanIndex index =
  ...
  where showPlanPackage p =
            showPlanPackageTag p ++ " "
                ++ display (packageId p) ++ " ("
                ++ display (installedSourceHash p) ++ ")"

showInstallPlan :: (HasComponentInstanceId ipkg, HasComponentInstanceId srcpkg)
...
--
new :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
        HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
    => Bool
...
new indepGoals index =
  ...
  let isPreExisting (PreExisting _) = True
      ...
      fakeMap = Map.fromList
              . map (\p -> (fakeComponentInstanceId (packageId p)
                           ,installedSourceHash p))
...
--
remove :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
           HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
...
ready plan = assert check readyPackages
  where
    ...
    isInstalledDep :: ComponentInstanceId -> Maybe ipkg
    isInstalledDep pkgid =
      ...
      case PlanIndex.fakeLookupComponentInstanceId
...
--
processing :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
               HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
           => [GenericReadyPackage srcpkg ipkg]
           -> GenericInstallPlan ipkg srcpkg iresult ifailure
           -> GenericInstallPlan ipkg srcpkg iresult ifailure
...
--
completed :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
              HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
          => ComponentInstanceId
          -> Maybe ipkg -> iresult
          -> GenericInstallPlan ipkg srcpkg iresult ifailure
  ...
  where
    plan'     = plan {
                  ...
                  planIndex = PackageIndex.insert installed
                            . PackageIndex.deleteComponentInstanceId pkgid
                            $ planIndex plan
                }
    ...
    insert_fake_mapping (Just ipkg) = Map.insert pkgid (installedSourceHash ipkg)
...
-- state.
--
failed :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
           HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
       => ComponentInstanceId         -- ^ The id of the package that failed to install
...
lookupProcessingPackage plan pkgid =
  ...
  -- planFakeMap
  case PackageIndex.lookupComponentInstanceId (planIndex plan) pkgid of
...
--
valid :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
          HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
      => FakeMap -> Bool
      -> PlanIndex ipkg srcpkg iresult ifailure
      -> Bool
...
--
problems :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
             HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
         => FakeMap -> Bool
         -> PlanIndex ipkg srcpkg iresult ifailure
...
problems fakeMap indepGoals index =
     ...
     [ PackageMissingDeps pkg
       (catMaybes
        (map
         (fmap packageId . PlanIndex.fakeLookupComponentInstanceId fakeMap index)
         missingDeps))
  ...
  ++ [ PackageStateInvalid pkg pkg'
     ...
     , Just pkg' <- map (PlanIndex.fakeLookupComponentInstanceId fakeMap index)
...
--
acyclic :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
            HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
...
--
closed :: (HasComponentInstanceId ipkg, PackageFixedDeps ipkg,
...
--
consistent :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
               HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
           => FakeMap -> PlanIndex ipkg srcpkg iresult ifailure -> Bool
...
dependencyClosure :: (HasComponentInstanceId ipkg,   PackageFixedDeps ipkg,
                      HasComponentInstanceId srcpkg, PackageFixedDeps srcpkg)
                  => GenericInstallPlan ipkg srcpkg iresult ifailure
                  -> [PackageIdentifier]
...
dependencyClosure installPlan pids =
    PlanIndex.dependencyClosure
      ...
      (map (resolveFakeId . fakeComponentInstanceId) pids)
  where
