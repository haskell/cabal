...
reportTargetSelectorProblems problems = do
    ...
    case [ (t, m, ms) | MatchingInternalError t m ms <- problems ] of
      ...
      ((target, originalMatch, renderingsAndMatches):_) ->
        die $ "Internal error in target matching. It should always be "
    ...
    case [ t | TargetSelectorNoCurrentPackage t <- problems ] of
      []       -> return ()
      target:_ ->
        die $ "The target '" ++ showTargetString target ++ "' refers to the "
    ...
    case [ () | TargetSelectorNoTargets <- problems ] of
      []  -> return ()
      _:_ ->
        die $ "No targets given and there is no package in the current "
