...
import Distribution.Client.Types
         ( SourcePackageDb(..), Repo, PackageLocation' )
...
getPkgList verbosity packageDBs repos comp conf listFlags pats = do
    ...
    let sourcePkgIndex = packageIndex sourcePkgDb
        ...
        pkgsInfo ::
          [(PackageName, [Installed.InstalledPackageInfo], [SourcePackage PackageLocation'])]
        pkgsInfo
...
sourcePkgsInfo ::
  ...
  -> (VersionRange, [Installed.InstalledPackageInfo], [SourcePackage loc])
sourcePkgsInfo prefs name installedPkgIndex sourcePkgIndex =
  (pref, installedPkgs, sourcePkgs)
  where
    pref          = prefs name
...
data PackageDisplayInfo = PackageDisplayInfo {
    ...
    selectedSourcePkg :: Maybe (SourcePackage PackageLocation'),
...
mergePackageInfo :: VersionRange
                 ...
                 -> Maybe (SourcePackage PackageLocation')
                 -> Bool
                 -> PackageDisplayInfo
