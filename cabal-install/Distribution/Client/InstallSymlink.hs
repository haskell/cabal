...
import Distribution.Package
         ( PackageIdentifier, Package(packageId), InstalledPackageId(..)
         , installedPackageId )
...
--      createDirectoryIfMissing True publicBinDir
      fmap catMaybes $ sequence
        ...
        | (ReadyPackage cfgpkg@(ConfiguredPackage _ _flags _ _) _, pkg, exe) <- exes
        , let pkgid  = packageId pkg
              -- This is a bit dodgy; probably won't work for Backpack packages
              ipid = installedPackageId cfgpkg
  ...
  where
    ...
    pkgBinDir :: PackageDescription -> InstalledPackageId -> IO FilePath
