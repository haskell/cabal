...
import Distribution.Package
         ( PackageName, PackageId, Package(..), ComponentInstanceId(..)
         , ComponentInstanceId(..)
...
class Package pkg => PackageFixedDeps pkg where
  depends :: pkg -> ComponentDeps [ComponentInstanceId]
...
-- 'ComponentInstanceId', we need to be able to synthesize these IDs prior
-- to installation.  Eventually, we'll move to a representation of
-- 'ComponentInstanceId' which can be properly computed before compilation
...
-- plan.
fakeComponentInstanceId :: PackageId -> ComponentInstanceId
fakeComponentInstanceId = ComponentInstanceId . (".fake."++) . display
...
--
-- Once we configure a source package we know it's ComponentInstanceId
...
-- and use it consistently instead of ComponentInstanceIds?
data ConfiguredId = ConfiguredId {
    confSrcId  :: PackageId
  , confInstId :: ComponentInstanceId
  }
...
instance HasComponentInstanceId ConfiguredPackage where
  installedSourceHash = fakeComponentInstanceId . packageId
...
instance (Package srcpkg, HasComponentInstanceId ipkg) =>
         PackageFixedDeps (GenericReadyPackage srcpkg ipkg) where
  depends (ReadyPackage _ deps) = fmap (map installedSourceHash) deps

instance HasComponentInstanceId srcpkg =>
         HasComponentInstanceId (GenericReadyPackage srcpkg ipkg) where
  installedSourceHash (ReadyPackage pkg _) = installedSourceHash pkg
