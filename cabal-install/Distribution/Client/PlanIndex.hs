...
import Distribution.Package
         ( PackageName(..), PackageIdentifier(..), InstalledUnitId(..)
...
import Distribution.Simple.PackageIndex
         ( PackageIndex, allPackages, insert, lookupInstalledUnitId )
import Distribution.Package
         ( HasInstalledUnitId(..), PackageId )

-- Note [FakeMap]
-----------------
-- We'd like to use the PackageIndex defined in this module for
-- cabal-install's InstallPlan.  However, at the moment, this
-- data structure is indexed by InstalledUnitId, which we don't
...
-- an InstalledUnitId will be calculatable prior to actually
...
-- is that, since we don't know what an InstalledUnitId is
...
-- it out with the actual generated InstalledUnitId after the
...
-- we update the InstalledUnitId.  We could map over the entire
...
type FakeMap = Map InstalledUnitId InstalledUnitId

-- | Variant of `depends` which accepts a `FakeMap`
--
-- Analogous to `fakeInstalledDepends`. See Note [FakeMap].
fakeDepends :: PackageFixedDeps pkg => FakeMap -> pkg -> ComponentDeps [InstalledUnitId]
fakeDepends fakeMap = fmap (map resolveFakeId) . depends
  where
    resolveFakeId :: InstalledUnitId -> InstalledUnitId
    resolveFakeId ipid = Map.findWithDefault ipid ipid fakeMap

--- | Variant of 'lookupInstalledUnitId' which accepts a 'FakeMap'.  See Note
--- [FakeMap].
fakeLookupInstalledUnitId :: FakeMap -> PackageIndex a -> InstalledUnitId
                             -> Maybe a
fakeLookupInstalledUnitId fakeMap index pkg =
  lookupInstalledUnitId index (Map.findWithDefault pkg pkg fakeMap)
...
brokenPackages fakeMap index =
  ...
  , let missing =
          [ pkg' | pkg' <- CD.nonSetupDeps (depends pkg)
                 , isNothing (fakeLookupInstalledUnitId fakeMap index pkg') ]
  , not (null missing) ]
...
-- for them here.
dependencyInconsistencies :: forall pkg. (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                          => FakeMap
                          -> Bool
                          -> PackageIndex pkg
...
rootSets :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
         => FakeMap -> Bool -> PackageIndex pkg -> [[InstalledUnitId]]
rootSets fakeMap indepGoals index =
...
libraryRoots :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
             => FakeMap -> PackageIndex pkg -> [InstalledUnitId]
...
setupRoots :: PackageFixedDeps pkg => PackageIndex pkg -> [[InstalledUnitId]]
...
dependencyInconsistencies' :: forall pkg.
                              (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                           => FakeMap
                           -> PackageIndex pkg
                           -> [(PackageName, [(PackageIdentifier, Version)])]
dependencyInconsistencies' fakeMap index =
  ...
  where
    ...
    inverseIndex :: Map PackageName (Map InstalledUnitId (pkg, [PackageId]))
    inverseIndex = Map.fromListWith (Map.unionWith (\(a,b) (_,b') -> (a,b++b')))
      ...
      , Just dep <- [fakeLookupInstalledUnitId fakeMap index ipid]
      ]
...
--
dependencyCycles :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                 => FakeMap
                 -> PackageIndex pkg
                 -> [[pkg]]
dependencyCycles fakeMap index =
  ...
  where
    adjacencyList = [ (pkg, installedUnitId pkg, CD.nonSetupDeps (fakeDepends fakeMap pkg))
                    | pkg <- allPackages index ]
...
dependencyClosure :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                  => FakeMap
                  -> PackageIndex pkg
                  -> [InstalledUnitId]
                  -> Either [(pkg, [InstalledUnitId])]
                            (PackageIndex pkg)
dependencyClosure fakeMap index pkgids0 = case closure mempty [] pkgids0 of
 ...
 where
    ...
    closure completed failed (pkgid:pkgids) =
      case fakeLookupInstalledUnitId fakeMap index pkgid of
        Nothing   -> closure completed (pkgid:failed) pkgids
        Just pkg  ->
          case fakeLookupInstalledUnitId fakeMap completed
...
topologicalOrder :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
...
reverseTopologicalOrder :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                        => FakeMap -> PackageIndex pkg -> [pkg]
...
--
reverseDependencyClosure :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                         => FakeMap
                         -> PackageIndex pkg
                         -> [InstalledUnitId]
                         -> [pkg]
...
--
dependencyGraph :: (PackageFixedDeps pkg, HasInstalledUnitId pkg)
                => FakeMap
                -> PackageIndex pkg
                -> (Graph.Graph,
                    Graph.Vertex -> pkg,
...
dependencyGraph fakeMap index = (graph, vertexToPkg, idToVertex)
  where
    ...
    vertexToPkg = fromJust
                . (\((), key, _targets) -> lookupInstalledUnitId index key)
                . vertexToPkg'
