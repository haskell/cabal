...
import Distribution.Package
         ( PackageName(..), PackageIdentifier(..), PackageKey(..)
...
import Distribution.Simple.PackageIndex
         ( PackageIndex, allPackages, insert, lookupPackageKey )
import Distribution.Package
         ( HasPackageKey(..), PackageId )

-- Note [FakeMap]
-----------------
-- We'd like to use the PackageIndex defined in this module for
-- cabal-install's InstallPlan.  However, at the moment, this
-- data structure is indexed by PackageKey, which we don't
...
-- an PackageKey will be calculatable prior to actually
...
-- is that, since we don't know what an PackageKey is
...
-- it out with the actual generated PackageKey after the
...
-- we update the PackageKey.  We could map over the entire
...
type FakeMap = Map PackageKey PackageKey

-- | Variant of `depends` which accepts a `FakeMap`
--
-- Analogous to `fakeInstalledDepends`. See Note [FakeMap].
fakeDepends :: PackageFixedDeps pkg => FakeMap -> pkg -> ComponentDeps [PackageKey]
fakeDepends fakeMap = fmap (map resolveFakeId) . depends
  where
    resolveFakeId :: PackageKey -> PackageKey
    resolveFakeId ipid = Map.findWithDefault ipid ipid fakeMap

--- | Variant of 'lookupPackageKey' which accepts a 'FakeMap'.  See Note
--- [FakeMap].
fakeLookupPackageKey :: FakeMap -> PackageIndex a -> PackageKey
                             -> Maybe a
fakeLookupPackageKey fakeMap index pkg =
  lookupPackageKey index (Map.findWithDefault pkg pkg fakeMap)
...
brokenPackages fakeMap index =
  ...
  , let missing =
          [ pkg' | pkg' <- CD.nonSetupDeps (depends pkg)
                 , isNothing (fakeLookupPackageKey fakeMap index pkg') ]
  , not (null missing) ]
...
-- for them here.
dependencyInconsistencies :: forall pkg. (PackageFixedDeps pkg, HasPackageKey pkg)
                          => FakeMap
                          -> Bool
                          -> PackageIndex pkg
...
rootSets :: (PackageFixedDeps pkg, HasPackageKey pkg)
         => FakeMap -> Bool -> PackageIndex pkg -> [[PackageKey]]
rootSets fakeMap indepGoals index =
...
libraryRoots :: (PackageFixedDeps pkg, HasPackageKey pkg)
             => FakeMap -> PackageIndex pkg -> [PackageKey]
libraryRoots fakeMap index =
    map (packageKey . toPkgId) roots
  where
...
setupRoots :: PackageFixedDeps pkg => PackageIndex pkg -> [[PackageKey]]
...
dependencyInconsistencies' :: forall pkg.
                              (PackageFixedDeps pkg, HasPackageKey pkg)
                           => FakeMap
                           -> PackageIndex pkg
                           -> [(PackageName, [(PackageIdentifier, Version)])]
dependencyInconsistencies' fakeMap index =
  ...
  where
    ...
    inverseIndex :: Map PackageName (Map PackageKey (pkg, [PackageId]))
    inverseIndex = Map.fromListWith (Map.unionWith (\(a,b) (_,b') -> (a,b++b')))
      ...
      , Just dep <- [fakeLookupPackageKey fakeMap index ipid]
      ]
...
--
dependencyCycles :: (PackageFixedDeps pkg, HasPackageKey pkg)
                 => FakeMap
                 -> PackageIndex pkg
                 -> [[pkg]]
dependencyCycles fakeMap index =
  ...
  where
    adjacencyList = [ (pkg, packageKey pkg, CD.nonSetupDeps (fakeDepends fakeMap pkg))
                    | pkg <- allPackages index ]
...
dependencyClosure :: (PackageFixedDeps pkg, HasPackageKey pkg)
                  => FakeMap
                  -> PackageIndex pkg
                  -> [PackageKey]
                  -> Either [(pkg, [PackageKey])]
                            (PackageIndex pkg)
dependencyClosure fakeMap index pkgids0 = case closure mempty [] pkgids0 of
 ...
 where
    ...
    closure completed failed (pkgid:pkgids) =
      case fakeLookupPackageKey fakeMap index pkgid of
        Nothing   -> closure completed (pkgid:failed) pkgids
        Just pkg  ->
          case fakeLookupPackageKey fakeMap completed
               (packageKey pkg) of
...
topologicalOrder :: (PackageFixedDeps pkg, HasPackageKey pkg)
...
reverseTopologicalOrder :: (PackageFixedDeps pkg, HasPackageKey pkg)
                        => FakeMap -> PackageIndex pkg -> [pkg]
...
--
reverseDependencyClosure :: (PackageFixedDeps pkg, HasPackageKey pkg)
                         => FakeMap
                         -> PackageIndex pkg
                         -> [PackageKey]
                         -> [pkg]
...
--
dependencyGraph :: (PackageFixedDeps pkg, HasPackageKey pkg)
                => FakeMap
                -> PackageIndex pkg
                -> (Graph.Graph,
                    Graph.Vertex -> pkg,
...
dependencyGraph fakeMap index = (graph, vertexToPkg, idToVertex)
  where
    ...
    vertexToPkg = fromJust
                . (\((), key, _targets) -> lookupPackageKey index key)
                . vertexToPkg'
