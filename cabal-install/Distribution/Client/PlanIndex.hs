...
import Distribution.Package
         ( PackageName(..), PackageIdentifier(..), ComponentInstanceId(..)
...
import Distribution.Simple.PackageIndex
         ( PackageIndex, allPackages, insert, lookupComponentInstanceId )
import Distribution.Package
         ( HasComponentInstanceId(..), PackageId )

-- Note [FakeMap]
-----------------
-- We'd like to use the PackageIndex defined in this module for
-- cabal-install's InstallPlan.  However, at the moment, this
-- data structure is indexed by ComponentInstanceId, which we don't
...
-- an ComponentInstanceId will be calculatable prior to actually
...
-- is that, since we don't know what an ComponentInstanceId is
...
-- it out with the actual generated ComponentInstanceId after the
...
-- we update the ComponentInstanceId.  We could map over the entire
...
type FakeMap = Map ComponentInstanceId ComponentInstanceId

-- | Variant of `depends` which accepts a `FakeMap`
--
-- Analogous to `fakeInstalledDepends`. See Note [FakeMap].
fakeDepends :: PackageFixedDeps pkg => FakeMap -> pkg -> ComponentDeps [ComponentInstanceId]
fakeDepends fakeMap = fmap (map resolveFakeId) . depends
  where
    resolveFakeId :: ComponentInstanceId -> ComponentInstanceId
    resolveFakeId ipid = Map.findWithDefault ipid ipid fakeMap

--- | Variant of 'lookupComponentInstanceId' which accepts a 'FakeMap'.  See Note
--- [FakeMap].
fakeLookupComponentInstanceId :: FakeMap -> PackageIndex a -> ComponentInstanceId
                             -> Maybe a
fakeLookupComponentInstanceId fakeMap index pkg =
  lookupComponentInstanceId index (Map.findWithDefault pkg pkg fakeMap)
...
brokenPackages :: (PackageFixedDeps pkg)
               => FakeMap
               -> PackageIndex pkg
               -> [(pkg, [ComponentInstanceId])]
brokenPackages fakeMap index =
  [ (pkg, missing)
  | pkg  <- allPackages index
  , let missing =
          [ pkg' | pkg' <- CD.nonSetupDeps (depends pkg)
                 , isNothing (fakeLookupComponentInstanceId fakeMap index pkg') ]
  , not (null missing) ]
...
-- for them here.
dependencyInconsistencies :: forall pkg. (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                          => FakeMap
                          -> Bool
                          -> PackageIndex pkg
...
rootSets :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
         => FakeMap -> Bool -> PackageIndex pkg -> [[ComponentInstanceId]]
rootSets fakeMap indepGoals index =
...
libraryRoots :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
             => FakeMap -> PackageIndex pkg -> [ComponentInstanceId]
libraryRoots fakeMap index =
    map (installedSourceHash . toPkgId) roots
  where
...
setupRoots :: PackageFixedDeps pkg => PackageIndex pkg -> [[ComponentInstanceId]]
...
dependencyInconsistencies' :: forall pkg.
                              (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                           => FakeMap
                           -> PackageIndex pkg
                           -> [(PackageName, [(PackageIdentifier, Version)])]
dependencyInconsistencies' fakeMap index =
  ...
  where
    ...
    inverseIndex :: Map PackageName (Map ComponentInstanceId (pkg, [PackageId]))
    inverseIndex = Map.fromListWith (Map.unionWith (\(a,b) (_,b') -> (a,b++b')))
      ...
      , Just dep <- [fakeLookupComponentInstanceId fakeMap index ipid]
      ]
    ...
    reallyIsInconsistent [p1, p2] =
      let pid1 = installedSourceHash p1
          pid2 = installedSourceHash p2
...
--
dependencyCycles :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                 => FakeMap
                 -> PackageIndex pkg
                 -> [[pkg]]
dependencyCycles fakeMap index =
  ...
  where
    adjacencyList = [ (pkg, installedSourceHash pkg, CD.nonSetupDeps (fakeDepends fakeMap pkg))
                    | pkg <- allPackages index ]
...
dependencyClosure :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                  => FakeMap
                  -> PackageIndex pkg
                  -> [ComponentInstanceId]
                  -> Either [(pkg, [ComponentInstanceId])]
                            (PackageIndex pkg)
dependencyClosure fakeMap index pkgids0 = case closure mempty [] pkgids0 of
 ...
 where
    ...
    closure completed failed (pkgid:pkgids) =
      case fakeLookupComponentInstanceId fakeMap index pkgid of
        Nothing   -> closure completed (pkgid:failed) pkgids
        Just pkg  ->
          case fakeLookupComponentInstanceId fakeMap completed
               (installedSourceHash pkg) of
...
topologicalOrder :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
...
reverseTopologicalOrder :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                        => FakeMap -> PackageIndex pkg -> [pkg]
...
--
reverseDependencyClosure :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                         => FakeMap
                         -> PackageIndex pkg
                         -> [ComponentInstanceId]
                         -> [pkg]
...
--
dependencyGraph :: (PackageFixedDeps pkg, HasComponentInstanceId pkg)
                => FakeMap
                -> PackageIndex pkg
                -> (Graph.Graph,
                    Graph.Vertex -> pkg,
                    ComponentInstanceId -> Maybe Graph.Vertex)
dependencyGraph fakeMap index = (graph, vertexToPkg, idToVertex)
  where
    ...
    vertexToPkg = fromJust
                . (\((), key, _targets) -> lookupComponentInstanceId index key)
                . vertexToPkg'
    ...
    edgesFrom pkg = ( ()
                    , resolve (installedSourceHash pkg)
