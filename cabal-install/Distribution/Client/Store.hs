...
newStoreEntry :: Verbosity
              ...
              -> CompilerId
              -> UnitId
              -> (FilePath -> IO a) -- ^ Action to place files in a temp dir
...
newStoreEntry verbosity storeDirLayout@StoreDirLayout{..}
    ...
    withTempIncomingDir storeDirLayout compid $ \incomingEntryDir -> do

      -- Write all store entry files into the temp dir.
...
withTempIncomingDir StoreDirLayout{storeIncomingDirectory} compid action = do
    createDirectoryIfMissing True (storeIncomingDirectory compid)
    bracket
      (createTempDirectory (storeIncomingDirectory compid) "new")
