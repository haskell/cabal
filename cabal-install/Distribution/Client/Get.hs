...
get verbosity repos globalFlags getFlags userTargets = do
  ...
  when (onlyPkgDescr && useFork) $
    warn verbosity $
      "Ignoring --source-repository for --only-package-description"
  if onlyPkgDescr
    then mapM_ (unpackOnlyPkgDescr verbosity prefix) pkgs
    else
      if useFork
        then fork pkgs
        else unpack pkgs
