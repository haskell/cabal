...
convIP idx ipi =
  let ipid = IPI.installedSourceHash ipi
...
convIPId :: PN -> SI.InstalledPackageIndex -> ComponentInstanceId -> Maybe (FlaggedDep () PN)
convIPId pn' idx ipid =
  case SI.lookupComponentInstanceId idx ipid of
    Nothing  -> Nothing
