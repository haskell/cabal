...
import Distribution.Client.Types
         ...
         , enableStanzas, ConfiguredId(..), fakeComponentInstanceId )
...
convertInstalledPackageIndex index' = PackageIndex.fromList
  ...
  where
    ...
    -- ComponentInstanceId, which means we do not directly know the corresponding
    -- source dependency. The only way to find out is to lookup the
    -- ComponentInstanceId to get the InstalledPackageInfo and look at its
    ...
    sourceDepsOf index ipkg =
      ...
      | let depids = InstalledPackageInfo.depends ipkg
            getpkg = InstalledPackageIndex.lookupComponentInstanceId index
      , (depid, mdep) <- zip depids (map getpkg depids) ]

    brokenPackageId (ComponentInstanceId str) =
...
finaliseSelectedPackages pref selected constraints =
  ...
  where
    ...
    confId pkg = ConfiguredId {
      ...
      , confInstId = fakeComponentInstanceId (packageId pkg)
      }
