...
import Distribution.Package
         ( PackageKey(..), PackageIdentifier(..), PackageId,
...
data SetupScriptOptions = SetupScriptOptions {
    ...
    useDependencies :: [(PackageKey, PackageId)],
...
externalSetupMethod verbosity options pkg bt mkargs = do
  ...
  cabalLibVersionToUse :: IO (Version, (Maybe PackageKey)
                             ,SetupScriptOptions)
  cabalLibVersionToUse = do
    ...
    where
      ...
      installedVersion :: IO (Version, Maybe PackageKey
                             ,SetupScriptOptions)
      installedVersion = do
    ...
    case PackageIndex.lookupDependency index cabalDep of
      ...
      pkgs -> let ipkginfo = head . snd . bestVersion fst $ pkgs
              in return (packageVersion ipkginfo
                        ,Just . packageKey $ ipkginfo, options'')
