...
import Distribution.Version
         ...
         , withinRange )
import Distribution.InstalledUnitInfo (installedUnitId)
import Distribution.Package
         ( InstalledUnitId(..), PackageIdentifier(..), PackageId,
...
data SetupScriptOptions = SetupScriptOptions {
    ...
    useDependencies :: [(InstalledUnitId, PackageId)],
...
externalSetupMethod verbosity options pkg bt mkargs = do
  ...
  cabalLibVersionToUse :: IO (Version, (Maybe InstalledUnitId)
                             ,SetupScriptOptions)
  cabalLibVersionToUse = do
    ...
    where
      ...
      installedVersion :: IO (Version, Maybe InstalledUnitId
                             ,SetupScriptOptions)
      installedVersion = do
  ...
  installedCabalVersion :: SetupScriptOptions -> Compiler -> ProgramConfiguration
                        -> IO (Version, Maybe InstalledUnitId
                              ,SetupScriptOptions)
  ...
  compileSetupExecutable :: SetupScriptOptions
                         -> Version -> Maybe InstalledUnitId -> Bool
                         -> IO FilePath
