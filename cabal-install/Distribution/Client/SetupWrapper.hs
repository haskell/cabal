...
import Distribution.Version
         ...
         , withinRange )
import Distribution.InstalledPackageInfo (installedSourceHash)
import Distribution.Package
         ( ComponentInstanceId(..), PackageIdentifier(..), PackageId,
...
data SetupScriptOptions = SetupScriptOptions {
    ...
    useDependencies :: [(ComponentInstanceId, PackageId)],
...
externalSetupMethod verbosity options pkg bt mkargs = do
  ...
  cabalLibVersionToUse :: IO (Version, (Maybe ComponentInstanceId)
                             ,SetupScriptOptions)
  cabalLibVersionToUse = do
    ...
    where
      ...
      installedVersion :: IO (Version, Maybe ComponentInstanceId
                             ,SetupScriptOptions)
      installedVersion = do
  ...
  installedCabalVersion :: SetupScriptOptions -> Compiler -> ProgramConfiguration
                        -> IO (Version, Maybe ComponentInstanceId
                              ,SetupScriptOptions)
    ...
    case PackageIndex.lookupDependency index cabalDep of
      ...
      pkgs -> let ipkginfo = head . snd . bestVersion fst $ pkgs
              in return (packageVersion ipkginfo
                        ,Just . installedSourceHash $ ipkginfo, options'')
  ...
  getCachedSetupExecutable :: SetupScriptOptions
                           -> Version -> Maybe ComponentInstanceId
                           -> IO FilePath
  ...
  compileSetupExecutable :: SetupScriptOptions
                         -> Version -> Maybe ComponentInstanceId -> Bool
                         -> IO FilePath
