...
import Distribution.Package
         ...
         , Package(..), ComponentInstanceId(..), ComponentInstanceId(..)
...
checkPrintPlan verbosity installed installPlan sourcePkgDb
  ...
  let oldBrokenPkgs =
          map Installed.installedSourceHash
        . PackageIndex.reverseDependencyClosure installed
        . map (Installed.installedSourceHash . fst)
  ...
  let newBrokenPkgs =
        filter (\ p -> not (Installed.installedSourceHash p `elem` excluded))
      ...
      (pkg:_) -> Just ((pkg, status), plan'')
        where
          ...
          ipkg   = Installed.emptyInstalledPackageInfo {
                     ...
                     Installed.installedSourceHash = pkgid
                   }
...
data PackageStatus = NewPackage
                   | NewVersion [Version]
                   | Reinstall  [ComponentInstanceId] [PackageChange]

type PackageChange = MergeResult PackageIdentifier PackageIdentifier

extractReinstalls :: PackageStatus -> [ComponentInstanceId]
...
packageStatus installedPkgIndex cpkg =
  case PackageIndex.lookupPackageName installedPkgIndex
    ...
    ps ->  case filter ((== packageId cpkg)
      ...
      pkgs@(pkg:_) -> Reinstall (map Installed.installedSourceHash pkgs)
                                (changes pkg cpkg)
  ...
  where
    ...
    resolveInstalledIds :: [ComponentInstanceId] -> [PackageIdentifier]
    resolveInstalledIds =
        nub
      . sort
      . map Installed.sourcePackageId
      . catMaybes
      . map (PackageIndex.lookupComponentInstanceId installedPkgIndex)
...
-- level for logging.
type UseLogFile = Maybe (PackageIdentifier -> ComponentInstanceId -> FilePath, Verbosity)
...
performInstallations verbosity
  ...
  where
    ...
    substLogFileName :: PathTemplate -> PackageIdentifier -> ComponentInstanceId -> FilePath
...
executeInstallPlan :: Verbosity
                   -> Compiler
                   -> JobControl IO (PackageId, ComponentInstanceId, BuildResult)
                   -> UseLogFile
                   -> InstallPlan
...
executeInstallPlan verbosity _comp jobCtl useLogFile plan0 installPkg =
  ...
  where
                 ...
                 spawnJob jobCtl $ do
                   ...
                   let ipid = case buildResult of
                                Right (BuildOk _ _ (Just ipi)) ->
                                     Installed.installedSourceHash ipi
                                _ -> ComponentInstanceId (display (packageId pkg))
    ...
    updatePlan pkgid (Right buildSuccess@(BuildOk _ _ mipkg)) =
        InstallPlan.completed (Source.fakeComponentInstanceId pkgid)
                              mipkg buildSuccess

    updatePlan pkgid (Left buildFailure) =
        InstallPlan.failed (Source.fakeComponentInstanceId pkgid)
    ...
    -- otherwise.
    printBuildResult :: PackageId -> ComponentInstanceId -> BuildResult -> IO ()
...
installReadyPackage platform cinfo configFlags
  ...
  installPkg configFlags {
    ...
    configDependencies = [ (packageName (Installed.sourcePackageId deppkg),
                            Installed.installedSourceHash deppkg)
                         | deppkg <- CD.nonSetupDeps deps ],
...
installUnpackedPackage verbosity buildLimit installLock numJobs
  ...
  ipid <- inDir workingDir
        $ Configure.computeComponentInstanceId pkg CLibName
  ...
  where
    ...
    addDefaultInstallDirs :: ComponentInstanceId -> ConfigFlags -> IO ConfigFlags
    ...
    maybeLogPath :: ComponentInstanceId -> IO (Maybe FilePath)
    maybeLogPath ipid =
      case useLogFile of
